...
const AetherKit=(()=>{
  ...
  function buildModelGroup(cls){
    ...
    // Per-instance materials so damage-flash / death fade don't bleed across units,
    // and tint each toward the class colour so classes sharing a base outfit look different.
    model.traverse(o=>{
      if(o.isMesh){
  ...
  function pose(rig,state,dt){
    ...
    try{
      ...
      if(state.spell){
        ...
        // wind-up -> follow-through with the right arm
        const s=state.meleeStrike||0; // 0..1..0
        const ang=(RIG_T.meleeWind - (RIG_T.meleeWind+RIG_T.meleeSwing)*s)*RIG_T.meleeSign;
        armPose(rig,'r', ang, -0.20);
        setBone(rig,'lowerarm_r', elbow + s*0.6, 0);
        armPose(rig,'l', armL);
        setBone(rig,'lowerarm_l', elbow, 0);
        setBone(rig,'spine_02', breathe + m*0.05, -s*0.25); // shoulder turn into the swing
      } else {
        armPose(rig,'l', armL);
        armPose(rig,'r', armR);
        setBone(rig,'lowerarm_l', elbow, 0);
...
const AB={
 ...
 shadow:[
  ...
  {name:'Umbral Pounce',icon:'🌑',type:'leap',school:'shadow',range:18,cast:0,cd:14,cost:24,value:90,tip:'Leap to your target and strike for 90 damage. Off the global cooldown. Grants Evasion for 4 sec, giving an 80% chance to dodge incoming melee attacks.'},
  {name:'Viper Cut',icon:'🦂',type:'dot',school:'shadow',range:3.9,cast:0,cd:9,cost:25,value:70,tip:'Deal 70 damage and poison the enemy for 6 sec. Consumes Venom Edge for a stronger strike and poison.'},
  ...
  {name:'Blind',icon:'👁️',type:'blind',school:'shadow',range:15,cast:0,cd:40,cost:15,value:5,tip:'Disorient the selected enemy for 5 sec. Breaks on damage and shares diminishing returns with Prism Hex and Lullaby Bloom. Long 40 sec setup cooldown.'},
  {name:'Ribbreaker',icon:'🩸',type:'singleStun',school:'physical',range:3.8,cast:0,cd:20,cost:30,value:35,tip:'A punishing off-GCD melee stun with 3.8m reach for 4 sec on a 20 sec cooldown. Landing it empowers your next Night Slash into Eviscerate for 45% more damage. After Smoke Veil, deploys Smoke Bomb.'},
 ...
 storm:[
  {name:'Arc Spark',icon:'⚡',type:'damage',school:'storm',range:25,cast:1.15,cd:0,cost:5,value:124,tip:'Cast a lightning bolt dealing 124 damage and restoring 4 mana. Slightly slower than before; each landed Arc Spark starts with a 10% chance to trigger Storm Surge.'},
  {name:'Forked Current',icon:'🔱',type:'chain',school:'storm',range:24,cast:0,cd:8,cost:7,value:132,tip:'Shock the target for 132 damage and arc to nearby enemies. Consuming Volcanic Overload restores additional mana and automatically hurls three smaller Lava Bursts at every enemy in your line of sight.'},
 ...
 soul:[
  {name:'Soul Scar',icon:'🕯️',type:'soulDot',school:'shadow',range:25,cast:0,cd:0,cost:4,value:31.5,tip:'Mark the target with soul decay, dealing 31.5 shadow damage each second for 15 sec.'},
  ...
  {name:'Essence Siphon',icon:'👻',type:'soulDrain',school:'shadow',range:24,cast:0,cd:12,cost:3,value:47,tip:'Channel for 2.5 sec, draining the target every 0.5 sec. Active afflictions strengthen each tick; its damage is slightly lower but its self-healing is 30% stronger.'},
  ...
  {name:'Soul Barrier',icon:'🔮',type:'shieldSelf',school:'shadow',range:0,cast:0,cd:28,cost:6,value:225,tip:'Surround yourself with a barrier absorbing 225 damage for 6 sec and become immune to interrupts for its duration.'},
  {name:'Fear',icon:'😱',type:'fear',school:'shadow',range:24,cast:1.15,cd:15,cost:3,value:3.5,tip:'Fear an enemy for 3.5 sec. Soulweaver fear does not break from periodic DoTs, but still breaks from direct/physical damage.'}
 ],
 pala:[
 ...
 warrior:[
  {name:'Mortal Swing',icon:'⚔️',type:'mortalSwing',school:'physical',range:4.0,cast:0,cd:0,cost:18,value:104,tip:'Strike for 104 damage. A successful Pummel empowers one Mortal Swing by 30%; Warbreaker and Avatar can amplify the same hit.'},
...
function setSpellIcon(element,name,fallback='✦'){if(element)element.innerHTML=spellIcon(name,fallback);}
...
const TALENT_UNLOCKED_ABILITIES={
 ...
 warrior:{war_execute_strike:{name:'Stormbolt',icon:'🔮',type:'stormbolt',school:'physical',range:22,cast:0,cd:25,cost:15,value:3,tip:'Talent ability. Hurl a violet storm bolt up to 22m. When the missile connects, it stuns the target for 3 sec and follows normal stun diminishing returns.'},war_rallying_wall:{name:'Rallying Wall',icon:'🚧',type:'warriorGuard',school:'physical',range:0,cast:0,cd:55,cost:0,value:0,tip:'Talent ability. Defensive emergency wall: reduces damage taken by 60% for 6 sec, but lowers your damage by 25% while active. Best used to survive enemy burst or when your healer is controlled.'},war_skullbreaker:{name:'Avatar',icon:'🗿',type:'avatar',school:'physical',range:0,cast:0,cd:60,cost:0,value:0,tip:'Talent ability. Increase all damage you deal by 16% for 10 sec. If you are rooted when Avatar is pressed, it removes the current root once, but does not grant root immunity afterwards.'},war_battle_banner:{name:'Battle Banner',icon:'🚩',type:'buff',school:'physical',range:0,cast:0,cd:45,cost:0,value:0,tip:'Talent ability. Battle banner for a pressure window.'}},
 storm:{storm_lava_burst:{name:'Volcanic Eruption',icon:'🌋',type:'volcanicEruption',school:'storm',range:24,cast:0,cd:0,cost:8,value:353,tip:'Instant off-global eruption dealing 353 damage, then automatically launching two Lava Bursts for roughly 60 damage each. Skybreaker Pulse readies exactly one use; the button remains unavailable until then.'},storm_grounding_aegis:{name:'Grounding Aegis',icon:'🌀',type:'shieldSelf',school:'storm',range:0,cast:0,cd:32,cost:10,value:320,tip:'Shield yourself for 320 damage, helping you survive while casting.'},storm_static_field:{name:'Static Field',icon:'🧲',type:'root',school:'storm',range:22,cast:0,cd:30,cost:14,value:3.5,tip:'Root the selected enemy.'},storm_chain_spark:{name:'Healing Surge',icon:'🔗',type:'heal',school:'storm',range:28,cast:1.5,cd:12,cost:10,value:340,tip:'Cast a strong surge that restores roughly 20% of a geared ally’s health before dampening.'}},
 soul:{soul_pandemic_bloom:{name:'Pandemic Bloom',icon:'🧫',type:'damage',school:'shadow',range:25,cast:0,cd:14,cost:8,value:274,tip:'Instantly deal 274 shadow damage. Shadowfury empowers the next Pandemic Bloom by 20%.'},soul_void_mend:{name:'Void Mend',icon:'💗',type:'soulDrain',school:'shadow',range:24,cast:0,cd:20,cost:6,value:45,tip:'Drain an enemy to damage them and stabilise yourself.'},soul_horror:{name:'Mortal Horror',icon:'😱',type:'fear',school:'shadow',range:20,cast:0,cd:15,cost:16,value:3.5,tip:'Fear the target for 3.5 sec and heal yourself for 20% maximum health before dampening. Periodic damage does not break this fear.'},soul_dark_pact:{name:'Dark Pact',icon:'🕳️',type:'shieldSelf',school:'shadow',range:0,cast:0,cd:40,cost:8,value:360,tip:'Surround yourself with a powerful shadow shield.'}},
 ...
 shadow:{shadow_sap:{name:'Sap',icon:'😵',type:'blind',school:'shadow',range:18,cast:0,cd:32,cost:18,value:4,tip:'Talent ability. Disorient an enemy for 4 sec, creating a clean setup window.'},shadow_shadowstep:{name:'Vendetta',icon:'🎯',type:'vendetta',school:'shadow',range:24,cast:0,cd:45,cost:6,value:0,tip:'Off-GCD mark for 8 sec. No flat damage bonus. During Vendetta, your Garrote, Internal Bleeding and rogue poisons/bleeds tick faster on the marked target.'},shadow_crimson_vial:{name:'Crimson Vial',icon:'🧪',type:'heal',school:'shadow',range:0,cast:0,cd:28,cost:0,value:185,tip:'Talent ability. Instant self-heal defensive.'},shadow_gouge:{name:'Gouge',icon:'👁️',type:'blind',school:'shadow',range:5,cast:0,cd:24,cost:16,value:3,tip:'Talent ability. Short disorient peel/setup.'}},
...
Object.assign(TALENT_UNLOCKED_ABILITIES.warrior,{
 ...
 war_disarm:{name:'Warbreaker',icon:'🪓',type:'warbreaker',school:'physical',range:4.5,cast:0,cd:30,cost:18,value:115,tip:'Talent ability. Strike for 115 damage and empower only your next Mortal Swing by 30%. The buff lasts up to 10 sec, then disappears immediately when Mortal Swing is pressed.'}
...
Object.assign(TALENT_UNLOCKED_ABILITIES.soul,{
 soul_shadowfury:{name:'Shadowfury',icon:'🌑',type:'singleStun',school:'shadow',range:20,cast:0,cd:36,cost:14,value:42,tip:'Talent ability. Short setup stun that empowers your next Pandemic Bloom by 20% for 8 sec.'},
...
const SUPPORTED_TALENT_ABILITY_TYPES=new Set(['damage','heal','hot','bigHeal','shield','shieldSelf','defensive','warriorGuard','monkDefensive','ironbark','natureSwiftness','undyingResolve','buff','blind','root','singleStun','windStun','leap','push','dot','livingBomb','combustion','flameShield','evasion','cloak','internalBleeding','vendetta','totemMastery','stormkeeper','frostShock','karma','tigereyeBrew','soulDrain','holyShock','windlordStrike','chiBurst','touchOfDeath','stormbolt','fistsChannel','whirlingDragonPunch','meteor','bladestorm','warbreaker','victoryRush','shiv','tigersLust','bestowFaith','ghanir','flameShock','volcanicEruption','avengingWings','archangel','darkArchangel','angelicBody','alterTime']);
...
function syncTalentUnlockedAbilities(){
 ...
 Object.keys(window.__AB_BASE).forEach(cls=>{
  ...
  Object.entries(defs).forEach(([talentId,ability])=>{if((learned[talentId]||0)>0&&ability.type!=='passiveOnly'&&!AB[cls].some(a=>a.name===ability.name)){const tuned={...ability,talentAbility:true};if(tuned.cost)tuned.cost=Math.max(1,Math.ceil(tuned.cost*.30));AB[cls].push(tuned);}});
 });
...
function isUntargetableStealth(u,viewer){return !!(u&&u.alive&&u.has&&u.has('stealth')&&viewer&&viewer.team!==u.team);} function crowdControlState(u){const order=['furyStun','cheapStun','fear','poly','sleep','blind','windIncap','stun','silence','root','lock_fire','lock_arcane','lock_nature','lock_storm','lock_physical','silence','slow'];for(const type of order){const e=u.effects.find(x=>x.type===type&&x.time>0); if(e)return {...effectMeta(type),type,time:e.time};}return null;} function centerControlState(u){const order=['furyStun','cheapStun','stun','fear','poly','sleep','blind','windIncap','root'];for(const type of order){const e=u.effects.find(x=>x.type===type&&x.time>0);if(e)return {...effectMeta(type),type,time:e.time};}return null;}
const CLASS_MECHANICS={
 flame:'Caster mechanic: Counterflare and Frostfire Nova are usable while moving and remain off-global. Counterflare grants two instant +20% Cinder Bolts. Meteor is a deliberate 20 sec burst cooldown that arms one off-global rapid +15% Ember Lance after landing. Ice Block is a 50 sec immunity/recovery defensive.',
 storm:'Storm mechanic: maintain the doubled Flame Shock, use Skybreaker Pulse to ready one Volcanic Eruption, and build Storm Surge through Arc Spark. Healing Surge adds a strong casted recovery tool, while Static Aegis now includes a 20% wall.',
 shadow:'Melee mechanic: Umbral Pounce grants 4 sec of Evasion against melee attacks. Ribbreaker arms Eviscerate, empowering the next Night Slash by 25%. Shadow Kick provides a reactive off-global interrupt.',
 sage:"Healer mechanic: G'Hanir empowers Blooming Echo and Rejuvenate by 50% and makes their ticks occur 50% faster for 7 sec without reducing their duration. Renewal Tide remains the emergency recovery button. Nature Swiftness now lets Lifesage choose between an immediate Renewal Tide or an instant Lullaby Bloom, while Ironbark also amplifies healing received.",
 pala:'Holy healer mechanic: Bestow Faith provides delayed recovery, Divine Toll fires three Holy Shocks, and Word of Glory is stronger. Paladin AI plans instants around current enemy procs instead of relying only on Holy Light.',
 ...
 wind:'Martial mechanic: Zephyr Palm has 3.5m range. Strike of the Windlord arms a glowing Rising Sun Kick proc. Fists of Fury is a cancellable moving channel that slows by 60%; putting it on cooldown unlocks the off-global Whirling Dragon Punch. Touch of Death records five seconds of damaging-spell pressure.',
 soul:'Affliction mechanic: stronger low-cost DoTs build into a slightly lighter but more restorative Essence Siphon. Three Unstable Afflictions are a serious pressure state, while Pandemic Bloom provides a larger payoff after the full rot is established.',
...
const CLASS_PASSIVES={
 flame:'Meteor: choose when to call the 20 sec Meteor cooldown instead of waiting for a passive proc. Once it lands, it arms one off-global rapid +15% Meteor Lance. Counterflare grants instant +20% Hot Streak Cinder Bolts. Ice Block restores up to 20% health while protecting you.',
 shadow:'Shadow Marks: three Night Slashes empower Viper Cut into Venom Edge. Umbral Pounce grants 80% melee Evasion for 4 sec. Ribbreaker readies Eviscerate for the next Night Slash. Veiled Assault arms Cheap Shot and Smoke Bomb. Shadow Kick interrupts nearby casts.',
 storm:'Storm Surge: Arc Spark begins at a 10% proc chance and gains +10% after every failure. Stormwarden regenerates mana 20% faster, uses slightly cheaper utility, and can shut down casts with Wind Shear. Triggering the proc restores 10 mana, Tempest Bolt hits restore 5 mana each, and empowered Forked Current restores 10 mana while firing three smaller Lava Bursts. Stormwarden remains below its original damage level after the recent tuning, while retaining the same proc-driven rotation.',
 wind:'Flow rewards sustained martial pressure. Touch of Death is off-global and detonates 30% of the damaging-spell damage loaded into its five-second mark. Strike of the Windlord arms Rising Sun Kick. Whirling Dragon Punch glows and becomes usable while Fists of Fury is on cooldown.',
 sage:'Playable Support: all main healing is strong and Renewal Tide is a large instant emergency button. Enemy targeting no longer automatically fires Verdant Bolt. Click the same enemy target again to deliberately enable healer auto-attacks, so Lullaby Bloom is not broken accidentally.',
 pala:'Beaconless Protector: Holy Shock can heal or damage and has a 35% critical chance to grant Infusion of Light. Enemy targeting no longer automatically swings; click the same enemy target again to deliberately enable Righteous Strike. Sacrifice grants Avenging Wings, Divine Steed improves mobility, and Hammer of Justice creates setup or peel windows.',
 ...
 soul:'Fluid Affliction: Soulweaver keeps its 0.5 sec global cooldown and heavy mana sustain. Soul Scar, Creeping Torment and Unstable Affliction now carry more of the damage profile; Essence Siphon deals slightly less damage but heals 30% more.',
...
const CLASS_GAME_PLANS={
 ...
 storm:{core:'Keep Flame Shock active and cast Arc Spark to build mana and Storm Surge procs.',burst:'Skybreaker Pulse stuns nearby enemies and readies one Volcanic Eruption. Use that eruption immediately on the kill target.',survival:'Static Aegis reduces damage. Frost Shock and Static Field help you kite. Healing Surge is recovery, not your main rotation.'},
...
const GEAR_STATS=['Stamina','Intellect','Agility','Strength','Power','Restoration','Vitality','Mana','Versatility'];
const CUSTOM_GEAR_STATS=['Power','Restoration','Vitality','Mana','Versatility'];
...
const GEAR_BUILD_INFO={
 ...
 warrior:{name:'Bladestorm Bruiser',stats:['Power','Vitality'],text:'Power drives Mortal Swing and Rend pressure; Vitality keeps you swinging through melee.'}
};
...
const RARITY_INFO={uncommon:{name:'Uncommon',colour:'#40c961'},rare:{name:'Rare',colour:'#408df5'},elite:{name:'Elite',colour:'#ef5459'},mythical:{name:'Mythical',colour:'#c47bff'}};
...
function rarityForIlvl(ilvl){return ilvl>=950?'mythical':ilvl>=935?'elite':ilvl>=920?'rare':'uncommon';}
...
function unlockEligibleMounts(p){p.mounts=Array.isArray(p.mounts)?p.mounts.filter(id=>MOUNT_CATALOGUE.some(m=>m.id)):[];p.mountSkins=p.mountSkins||{};p.activeMountSkins=p.activeMountSkins||{};
 ...
 const highest=Math.max(1600,...Object.values(p.ratings?.['2v2']||{}),...Object.values(p.ratings?.['3v3']||{}));MOUNT_CATALOGUE.filter(m=>m.threshold&&highest>=m.threshold).forEach(m=>{if(!p.mounts.includes(m.id))p.mounts.push(m.id);});
...
function aetherLearnTalentChoice(cls,node,tree,state,rank,nodeCanLearn){
 ...
 if((node.virtualChoice||node.choice)&&siblings.length>1){
...
function aetherBasicTalentTree(cls='flame'){
 ...
 wrap.innerHTML=`<svg class="talent-lines">${lines}</svg>`+displayNodes.map(n=>{const r=displayRank(n),max=displayMax(n),learned=r>0,can=displayCanLearn(n);return `<button class="talent-node ${n.unlockAbility?'unlock-node':n.capstone?'capstone-node':'passive-node'} ${n.choice||n.virtualChoice?'choice choicehub':''} ${learned?'learned refundable':can?'available':'locked'}" style="left:${n.x}%;top:${n.y}%;" data-basic-talent="${n.id}"><span class="talent-icon">${talentIcon(n)}</span><span class="talent-rank">${r}/${max}</span></button>`;}).join('');
...
function enhanceTalentTreeNodes(){
 ...
 const common=[
  ...
  ['warrior',{id:'war_plate_training',name:'Plate Training',icon:'🛡️',x:50,y:92,max:2,req:['brutalchoice'],staminaNode:true,effects:{staminaPct:3},desc:'Increases maximum health by 3% per rank. This gives Warrior a sturdier bruiser profile without adding more burst damage.'}],
 ...
 [
  ...
  ['warrior',{id:'war_victory_rush',name:'Victory Rush',icon:'🏆',x:84,y:74,max:1,req:['war_plate_training'],unlockAbility:true,desc:'Unlocks Victory Rush: light enemy damage and a self-heal for sustain.'}],
  ['warrior',{id:'war_disarm',name:'Warbreaker',icon:'🪃',x:84,y:92,max:1,req:['war_plate_training'],unlockAbility:true,desc:'Unlocks Warbreaker: deal damage and increase your next Mortal Swing by 20%.'}],
...
function applyV221TalentText(){
 ...
 update('warrior','war_disarm',{desc:'Unlocks Warbreaker: deal damage and increase your next Mortal Swing by 30%.'});
...
function earnedTalentPoints(cls){const r=Math.max(classRating(cls,'2v2'),classRating(cls,'3v3'));return Math.max(26,Math.min(48,26+Math.floor(Math.max(0,r-1600)/90)+Math.floor((progression.wins||0)/30)));}
function availableTalentPoints(cls){return Math.max(0,earnedTalentPoints(cls)-spentTalentPoints(cls));}
function talentNodeAvailable(cls,node){const state=classTalentState(cls);if((state[node.id]||0)>=node.max)return false;if(node.req&&node.req.length&&!node.req.some(id=>(state[id]||0)>0))return false;if(node.choice&&talentTree(cls).some(n=>n.choice===node.choice&&n.id!==node.id&&(state[n.id]||0)>0))return false;return availableTalentPoints(cls)>0;}
...
function classTalentDamageMult(cls,c=null,t=null,label=''){
 let m=1;
 try{
  m+=talentTree(cls).reduce((x,n)=>x+(talentRank(cls,n.id)*(n.effects?.damagePct||0))/100,0);
 }catch(err){
  console.warn('Talent damage fallback',cls,err);
  m=1;
 }
 if(cls==='flame'&&/Meteor Lance|Ember Lance/.test(label))m+=talentRank(cls,'meteorimpact')*.04;
 if(cls==='warrior'&&/Mortal Swing/.test(label)&&t&&t.hp/t.maxHp<.35)m+=talentRank(cls,'executioner')*.05;
 if(cls==='storm'&&/Volcanic|Forked|Lava|Tempest/.test(label))m+=talentRank(cls,'lavacore')*.04;
 if(cls==='wind'&&/Fists/.test(label))m+=talentRank(cls,'focusfury')*.04;
 if(c?.has?.('totemMastery')){
  m*=1.05/1.03;
  if(/Flame Shock/i.test(label))m*=1.10;
 }
 if(cls==='warrior'&&/Mortal Swing/.test(label)&&c?.has?.('warbreakerDamage')){
  m*=13/12;
  c.effects=c.effects.filter(e=>e.type!=='warbreakerDamage');
 }
 if(cls==='wind'&&c)m*=windTigereyeBuffMult(c);
 return Number.isFinite(m)&&m>0?m:1;
}
function classTalentHealingMult(cls,c=null){let m=1+talentTree(cls).reduce((x,n)=>x+(talentRank(cls,n.id)*(n.effects?.healingPct||0))/100,0);if(cls==='wind'&&c)m*=windTigereyeBuffMult(c);if(c?.has?.('totemMastery'))m*=1.05/1.03;return m;}function classTalentStaminaMult(cls){return 1+talentTree(cls).reduce((m,n)=>m+(talentRank(cls,n.id)*(n.effects?.staminaPct||0))/100,0);}window.TALENT_TREES=TALENT_TREES;

function defaultProgression(){
 const ratings={'2v2':{},'3v3':{}},equipped={};GEAR_CLASSES.forEach(c=>{ratings['2v2'][c]=1600;ratings['3v3'][c]=1600;equipped[c]={};});
 return {shards:900,ratings,equipped,inventory:[],matches:0,wins:0,tournaments:0,tournamentCupWins:{},tournamentCupRewarded:{},achievements:{},achievementClasses:{},currentTitle:'',mounts:baselineMountIds(),activeMount:'skyhoof',mountSkins:{aethergladiatorwyrm:['default']},activeMountSkins:{aethergladiatorwyrm:'default'},talents:{},settings:{hideCombatText:false,hideLiveDetails:false,stackingNameplates:true,showTargetOfTarget:true}};
}
const ACHIEVEMENTS=[
...
function achievementEligibleForClass(def,cls=currentTitleClass()){
 ...
 if(def.type==='rating')return Math.max(classRating(cls,'2v2'),classRating(cls,'3v3'))>=def.threshold;
...
function loadProgression(){
 ...
 if(!p.ratings||!p.ratings['2v2']||!p.ratings['3v3']){const source=oldRatings||{};p.ratings={'2v2':{},'3v3':{}};GEAR_CLASSES.forEach(c=>{const start=Number.isFinite(source[c])?source[c]:1600;p.ratings['2v2'][c]=start;p.ratings['3v3'][c]=start;});}
 p.inventory=p.inventory.map(item=>({...item,rarity:item.rarity||rarityForIlvl(item.ilvl),name:gearName(item.classKey,item.slot,item.ilvl),flavour:item.flavour||gearFlavour(item.classKey)}));
 GEAR_CLASSES.forEach(c=>{if(!Number.isFinite(p.ratings['2v2'][c]))p.ratings['2v2'][c]=1600;if(!Number.isFinite(p.ratings['3v3'][c]))p.ratings['3v3'][c]=1600;if(!p.equipped[c])p.equipped[c]={};});
...
function highestArenaRating(){return Math.max(...['2v2','3v3'].flatMap(mode=>GEAR_CLASSES.map(c=>classRating(c,mode))));}
...
function blankStats(){return {Stamina:0,Intellect:0,Agility:0,Strength:0,Power:0,Restoration:0,Vitality:0,Mana:0,Versatility:0};}
function itemStatValues(item){
 const stats=blankStats(),mainStat=CLASS_PRIMARY[item.classKey]||'Strength';
 // Core Stamina/primary are automatic. Build Bonuses are exactly the two styles selected by the player.
 const stamina=Math.round(8+(item.ilvl-910)*.28),primary=Math.round(7+(item.ilvl-910)*.30),styleValue=Math.round(10+(item.ilvl-910)*.47);
...
class Character {
 ...
 effect(type,duration,data={}){
  ...
  if(type==='warbreakerReady'){
   data={...data,pct:.30};
   const amp=this.effects.find(e=>e.type==='warbreakerDamage');
   if(amp)amp.time=duration;else this.effects.push({type:'warbreakerDamage',time:duration,tick:1});
  }
  const effectKey=data.effectKey||'';
  const existing=this.effects.find(e=>e.type===type&&(!effectKey||(e.effectKey||'')===effectKey));
  if(existing){existing.time=duration;Object.assign(existing,data);return existing;}
 ...
 removeDispellable(){const order=['poly','sleep','blind','windIncap','fear','root','slow','livingBomb','karmaDot'];for(const type of order){const i=this.effects.findIndex(e=>e.type===type);if(i>=0){this.effects.splice(i,1);return type;}}return null;}
 update(dt){if(!this.alive)return;if(this.gearAppearance?.userData?.tick)this.gearAppearance.userData.tick(dt);this.gcd=Math.max(0,this.gcd-dt);this.basicAttackCd=Math.max(0,(this.basicAttackCd||0)-dt);this.trinketCd=Math.max(0,this.trinketCd-dt);this.tempestLock=Math.max(0,(this.tempestLock||0)-dt);for(let i=0;i<this.cds.length;i++)this.cds[i]=Math.max(0,this.cds[i]-dt);
  ...
  const smoke=this.has('defensive');const meleeBase=5.15,isCaster=['flame','storm','soul'].includes(this.cls),isHealer=['sage','pala','disc'].includes(this.cls);this.moveSpeed=isCaster?meleeBase*1.15:isHealer?meleeBase*1.05:meleeBase;if(smoke&&this.cls==='shadow')this.moveSpeed=6.75;if(this.mounted)this.moveSpeed*=1.42;if(this.has('tigersLust'))this.moveSpeed*=Number(this.has('tigersLust').speed||1.70);if(this.has('divineSteed'))this.moveSpeed=(meleeBase*1.05)*1.65;if(this.has('discFade'))this.moveSpeed*=Number(this.has('discFade').speed||1.25);if(this.has('angelicBody'))this.moveSpeed*=Number(this.has('angelicBody').speed||1.30);if(this.has('cauterizeDoom'))this.moveSpeed*=1.50;if(this.modelGroup)this.modelGroup.scale.setScalar(this.has('avatar')?1.24:1);
  const poly=!!this.has('poly');
  const frameDistance=Math.hypot(this.x-this.lastX,this.z-this.lastZ);
  const immobilised=poly||this.has('sleep')||this.has('blind')||this.has('windIncap')||this.has('fear')||this.has('iceBlock')||this.has('stun')||this.has('cheapStun')||this.has('furyStun')||this.has('root')||!!(this.cast&&!this.cast.moveCast);
  ...
  if(this.combatAnim&&this.combatAnim.until>this.game.time&&!activeCast){
    ...
    if(this.combatAnim.type==='melee'){
      this.armR.rotation.x=-1.75*strike;this.armR.rotation.z=-.18-.35*strike;
      this.handR.position.set(.47,1.05,-.62*strike);this.torso.rotation.z=-.10*strike;
    }else{
  ...
  if(this.modelGroup)this.modelGroup.position.y=this.mountVisual.visible?(this.mountData?.riderY||.9):0;
...
Character.prototype.takeDamage=function(source,amount,label){
 ...
 const result=aetherBaseTakeDamage.call(this,source,amount,label);
...
class AIController {
 constructor(game,unit){
  ...
  this.moveIntent=null;this.route=null;this.stuckTime=0;this.lastMoveX=unit.x;this.lastMoveZ=unit.z;this.openerHold=2.2+Math.random()*1.8;
 }
 ratingProfile(base){
 ...
 update(dt){
  syncTalentUnlockedAbilities();const u=this.u;if(!u.alive||this.game.phase!=='fight'||this.game.paused)return;
  ...
  if(u.cast&&u.cast.moveCast){
   ...
   if(u.cls==='sage'&&enemiesForCast.length){
    ...
    if(t&&t.alive){if(t.team===u.team&&dist(u,t)>20)this.moveToward(t,false);else if(t.team!==u.team&&dist(u,t)>21)this.moveToward(t,false);}
   }
   return;
  }
 ...
 reactiveCastCancelNeeded(target,drop,burst){
  ...
  const sameUsefulTarget=castTarget===target&&['heal','holyLight','bigHeal'].includes(castType);
 ...
 tryTeamPeel(allies,enemies,d){
  ...
  const controls={flame:['Prism Hex','Frostfire Nova'],warrior:['Stormbolt','Intimidating Shout'],wind:['Incapacitate','Valley Sweep'],shadow:['Blind','Ribbreaker'],storm:['Static Snare','Static Field']}[u.cls]||[];
 ...
 dpsSurvivalResponse(allies,enemies,d){
  ...
  if(!this.game.majorDefensiveActive?.(u)){
   ...
   if(u.cls==='soul'&&(hp<.62||realMajor&&hp<.74))this.selfSaveAbility(['Undying Resolve','Dark Pact','Void Mend']);
  }
 ...
 tryTalentToolkit(enemies){
   ...
   const names=[
    ...
    'Undying Resolve','Dark Pact','Void Mend','Pandemic Bloom','Mortal Horror','Shadowfury',
    'Grounding Aegis','Totem Mastery','Stormkeeper','Volcanic Eruption','Healing Surge','Frost Shock','Static Field','Thunderstep','Mana Well',
    'Rallying Wall','Battle Banner','Warbreaker','Avatar','Stormbolt','Victory Rush','Bladestorm'
   ];
   for(const name of names){
    ...
    if(['Fire Shield','Molten Armor','Alter Time'].includes(name)){
    ...
    }else if(['Crimson Vial','Void Mend','Victory Rush'].includes(name)){
     if(!(hp<.66||healerLocked&&hp<.78))continue; target=name==='Void Mend'?t:u; self=name!=='Void Mend';
     ...
     self=true;target=u;
    }else if(['Dark Pact','Grounding Aegis','Rallying Wall'].includes(name)){
    ...
    }else if(name==='Battle Banner'||name==='Mana Well'){
    ...
    }else if(name==='Meteor Spear'||name==='Meteor'||name==='Volcanic Eruption'||name==='Pandemic Bloom'){
     if(!los||td>(a.range||24)+.6)continue;
     if(name==='Volcanic Eruption'&&!u.has('volcanicEruptionReady'))continue;
     if(!pressureWindow&&thp>.88&&['Meteor Spear','Meteor','Volcanic Eruption','Pandemic Bloom'].includes(name))continue;
     target=t;
     ...
     const cc=pickCCTarget(a.range||22,'root')||t; if(!cc||!los||td>22)continue; target=cc;
  ...
  if(this.tryPlannedControl(enemies,t,d,!!killTarget))return;
  ...
  if(u.cls==='shadow'){
    ...
    if(closeCaster&&u.cds[6]<=0){this.game.tryAbility(u,6,closeCaster);return;}
    ...
    if(!u.has('smokePower')&&u.cds[3]<=0&&u.cds[5]<=0&&t.hp/t.maxHp<.88){this.game.tryAbility(u,3,u);return;}
    ...
    if(vend>=0&&u.cds[vend]<=0&&dist(u,t)<=24&&this.game.arena.los(u,t)&&t.hp/t.maxHp<.78&&!t.has('vendetta')){this.game.tryAbility(u,vend,t);return;}
    ...
    if(u.cds[5]<=0&&t.hp/t.maxHp<.7&&this.ccValue(t,'stun')>0){this.game.tryAbility(u,5,t);return;}
  ...
  if(u.cls==='soul'){
    ...
    if(!t.has('agony')){this.game.tryAbility(u,1,t);return;}
    const ua=t.has('unstableAffliction');
    if(!ua||(ua.stacks||1)<3){this.game.tryAbility(u,2,t);return;}
    ...
    if(t.has('soulScar')&&t.has('agony')&&(ua.stacks||0)>=3&&u.cds[3]<=0){this.game.tryAbility(u,3,t);return;}
  ...
  if(u.cls==='storm'){
    ...
    const flameShock=idx('Flame Shock'),lava=idx('Volcanic Eruption'),chain=idx('Healing Surge'),stormkeeper=idx('Stormkeeper'),totem=idx('Totem Mastery'),frost=idx('Frost Shock'),skybreaker=idx('Skybreaker Pulse'),staticField=idx('Static Field'),thunderstep=idx('Thunderstep');
    ...
    if(staticField>=0&&u.cds[staticField]<=0&&dist(u,t)<=22&&dist(u,t)>7&&!t.has('root')&&!t.has('slow')&&(selfHp<.68||setup)){this.game.tryAbility(u,staticField,t);return;}
  ...
  if(u.cls==='warrior'){
   ...
   const wall=AB[u.cls].findIndex(a=>a.name==='Rallying Wall');
   if(u.hp/u.maxHp<.4&&wall>=0&&u.cds[wall]<=0){this.game.tryAbility(u,wall,u);return;}
 ...
 survivorHealerDuel(allies,enemies){
  ...
  if(u.cls==='disc'){
   ...
   if(penance>=0&&u.cds[penance]<=0&&u.resource>=8&&los&&range<=25){this.stopMove();this.game.tryAbility(u,penance,foe);return true;}
   if(solace>=0&&u.cds[solace]<=0&&los&&range<=25){this.stopMove();this.game.tryAbility(u,solace,foe);return true;}
 ...
 paladinHealer(allies,enemies,d){const u=this.u;if(this.antiMageHealerResponse(allies,enemies))return;const healable=allies.filter(a=>!a.has('smokeBomb'));const low=(healable.length?healable:allies).slice().sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];const hp=low.hp/low.maxHp;const injured=allies.filter(a=>a.hp/a.maxHp<.74).length;const controlledAlly=allies.filter(a=>a!==u&&a.alive&&a.effects.some(e=>['poly','sleep','blind','windIncap','fear','root','slow'].includes(e.type))).sort((a,b)=>{const score=x=>x.has('poly')||x.has('sleep')||x.has('blind')||x.has('windIncap')||x.has('fear')?5:x.has('root')?3:1;return score(b)-score(a);})[0];const dispelTarget=allies.filter(a=>a.alive&&dist(u,a)<=28&&this.game.arena.los(u,a)&&a.effects.some(e=>['poly','sleep','blind','windIncap','fear','root','slow'].includes(e.type))).sort((a,b)=>{const score=x=>x.has('poly')||x.has('sleep')||x.has('blind')||x.has('windIncap')||x.has('fear')?5:x.has('root')?3:1;return score(b)-score(a);})[0];const threats=enemies.filter(e=>e.alive).sort((a,b)=>dist(a,u)-dist(b,u));const threat=threats[0];const rangedPressure=threats.find(e=>e.alive&&((e.cast&&e.cast.target===u)||(this.attacking(u,e)&&dist(e,u)<=27&&this.game.arena.los(e,u))));const targeted=threats.some(e=>(e===this.game.player&&this.game.target===u)||(e.ai&&e.ai.focus===u));const peelThreat=this.peelThreat(enemies,allies,10);const danger=!!(threat&&(dist(threat,u)<10||targeted)||rangedPressure);const partnerLocked=allies.find(a=>a!==u&&a.alive&&['poly','fear','sleep','blind','windIncap','furyStun','cheapStun','stun'].some(type=>a.has(type)));const incomingCC=enemies.find(e=>e.alive&&e.cast&&e.cast.target===u&&['poly','sleep','fear'].includes(e.cast.a?.type));const swapThreat=threats.find(e=>e.alive&&!['sage','pala','disc'].includes(e.cls))||threat;const isolated=!!(partnerLocked&&swapThreat&&(targeted||dist(swapThreat,u)<18||this.attacking(u,swapThreat)||u.hp/u.maxHp<.90));
  ...
  if(teammateBurst&&sacrifice>=0&&u.cds[sacrifice]<=0&&!low.has('sacrifice')&&u.resource>=(AB[u.cls][sacrifice].cost||0)){this.stopMove();this.game.tryAbility(u,sacrifice,low);return;}
  ...
  if(!danger&&hp>.88&&threat&&judgement>=0&&u.cds[judgement]<=0&&dist(u,threat)<=24&&this.game.arena.los(u,threat)){this.stopMove();this.game.tryAbility(u,judgement,threat);return;}
 ...
 disciplineHealer(allies,enemies,d){
  ...
  const idx=name=>AB[u.cls].findIndex(a=>a.name===name);
  const shield=idx('Power Shield'),penance=idx('Penance'),mend=idx('Shadow Mend'),solace=idx('Solace'),pain=idx('Pain Suppression'),radiance=idx('Ultimate Radiance'),purify=idx('Purify'),fear=idx('Psychic Scream'),fade=idx('Fade'),archangel=idx('Archangel'),darkArch=idx('Dark Archangel'),angelicBody=idx('Angelic Body');
  ...
  const burstScore=this.enemyBurstPotential(low,enemies),holyLocked=!!u.has('lock_holy');
  const injured=allies.filter(a=>a.alive&&a.hp/a.maxHp<.58).length;
  if((hp<.52||burst&&hp<.74)&&pain>=0&&u.cds[pain]<=0&&!low.has('painSuppression')){if(u.cast){u.cast=null;u.gcd=0;}this.game.tryAbility(u,pain,low);return;}
  if((hp<.36||injured>=2)&&radiance>=0&&u.cds[radiance]<=0&&u.resource>=(AB[u.cls][radiance].cost||0)){if(u.cast){u.cast=null;u.gcd=0;}this.game.tryAbility(u,radiance,u);return;}

  if(selfHp<.52&&fade>=0&&u.cds[fade]<=0){this.game.tryAbility(u,fade,u);return;}
  ...
  const needsAtonement=allies.filter(a=>a.alive&&atonementTime(a)<3).sort((a,b)=>this.healUrgency(b,enemies)-this.healUrgency(a,enemies))[0];
  if(shield>=0&&u.cds[shield]<=0&&needsAtonement&&u.resource>=(AB[u.cls][shield].cost||0)&&(needsAtonement===low||needsAtonement.hp/needsAtonement.maxHp<.92)&&!needsAtonement.has('shield')){this.game.tryAbility(u,shield,needsAtonement);return;}

  const damageTargets=enemies.filter(e=>e.alive&&this.game.arena.los(u,e)&&dist(u,e)<=25).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp);
  const coordinated=this.coordinatedTarget(enemies);
  const target=coordinated&&damageTargets.includes(coordinated)?coordinated:damageTargets[0];
  ...
  const mendEmergency=hp<.40||(holyLocked&&hp<.56)||(hp<.50&&burstScore>=3.2&&!penanceReady);
  ...
  if(darkArch>=0&&u.cds[darkArch]<=0&&atoned>=1&&target&&hp>.72&&(target.hp/target.maxHp<.72||this.game.unitUnderMajorOffensive(target)||this.enemyHealerState(enemies).locked)){this.game.tryAbility(u,darkArch,u);return;}
  if(atoned&&canDamage){
   if(penanceReady&&(hp<.96||burst||atoned>=2)){this.stopMove();this.game.tryAbility(u,penance,target);return;}
   if(solace>=0&&u.cds[solace]<=0&&dist(u,target)<=25&&this.game.arena.los(u,target)){this.game.tryAbility(u,solace,target);return;}
   if(hp<.97&&smite>=0&&u.resource>=(AB[u.cls][smite].cost||0)){this.stopMove();this.game.tryAbility(u,smite,target);return;}
  }
  ...
  if(!canDamage&&hp<.48&&penanceReady&&dist(u,low)<=25&&this.game.arena.los(u,low)){this.game.tryAbility(u,penance,low);return;}
  if(!canDamage&&hp<.55&&mend>=0&&u.resource>=(AB[u.cls][mend].cost||0)){this.stopMove();this.game.tryAbility(u,mend,low);return;}
  const close=enemies.find(e=>e.alive&&dist(u,e)<=8&&this.game.arena.los(u,e));
  if(close&&fear>=0&&u.cds[fear]<=0&&this.ccValue(close,'fear')>0){this.game.tryAbility(u,fear,u);return;}
  this.healerReposition(allies,enemies,low,{threat:enemies[0],d});
 }
...
class AudioManager {
 ...
 playAbilityUnspatial(a,c=null){
  ...
  if(cls==='soul'){
   if(/Unstable Affliction/i.test(n))return this.play('soulVoid');
   ...
   if(/Pandemic Bloom|Essence Siphon|Void Mend/i.test(n)||t==='soulDrain')return this.play('soulBloom');
...
class Game {
 ...
 bindUI(){
  ...
  $('#loadoutClass').onchange=()=>{this.selectArmouryClass($('#loadoutClass').value);this.renderArmoury();};
 ...
 closeArmoury(){$('#armoury').classList.add('hidden');$('#gearTooltip').classList.add('hidden');this.armouryDragging=false;this.refreshMenuProgress();}
 initArmouryPreview(){
...
const addMythicLegend=(slot,style)=>{
 ...
 updateArmouryPreview(dt){
  ...
  const canvas=$('#armouryCanvas'),w=Math.max(1,canvas.clientWidth),h=Math.max(1,canvas.clientHeight);
  if(canvas.width!==Math.floor(w*Math.min(devicePixelRatio*1.5,3))||canvas.height!==Math.floor(h*Math.min(devicePixelRatio*1.5,3))){this.armouryRenderer.setSize(w,h,false);this.armouryCamera.aspect=w/h;this.armouryCamera.updateProjectionMatrix();}
 ...
 renderArmoury(){
  ...
  $('#armClassTabs').querySelectorAll('.arm-class-tab').forEach(btn=>btn.onclick=()=>{this.selectArmouryClass(btn.dataset.class);this.renderArmoury();});
  this.selectArmouryClass(loadCls);
  const totals=this.getEquippedStats(loadCls),hpBonus=Math.min(420,Math.round((totals.Vitality||0)*.72+(totals.Stamina||0)*.72)),manaBonus=Math.min(60,Math.round((totals.Mana||0)*.18)),outDmg=(Math.min(.30,(totals.Power||0)*.00038+(totals.Versatility||0)*.00018+((totals.Intellect||0)+(totals.Agility||0)+(totals.Strength||0))*.00010)*100).toFixed(1),outHeal=(Math.min(.30,(totals.Restoration||0)*.00038+(totals.Versatility||0)*.00018+((totals.Intellect||0)+(totals.Agility||0)+(totals.Strength||0))*.00010)*100).toFixed(1);
  const core=this.classCoreStats(loadCls),totalStamina=core.stamina+(totals.Stamina||0),totalPrimary=core.primaryValue+(totals[core.primary]||0);$('#equippedStats').innerHTML=`<strong>${CLASS_INFO[loadCls].badge} ${CLASS_INFO[loadCls].name}</strong> · ${bracket.toUpperCase()} Rating ${classRating(loadCls,bracket)}<br><span>Core Attributes:</span> ${totalStamina} Stamina <small>(+${totals.Stamina||0} gear)</small> · ${totalPrimary} ${core.primary} <small>(+${totals[core.primary]||0} gear)</small><br>${this.formatStatLine(totals)}<br><span>Bonus Effects:</span> +${hpBonus} HP · +${manaBonus} Mana · +${outDmg}% Damage · +${outHeal}% Healing<div class="equipped-tools"><button id="openStatGuideBtn" class="small-arm-btn">Stat Guide</button></div>`;
 ...
 gearCard(item,equipped=false,inventory=false){
   const values=itemStatValues(item),sell=Math.floor(item.price*.45),rarity=rarityForIlvl(item.ilvl),quality=RARITY_INFO[rarity],next=gearUpgradeStep(item.ilvl),upCost=gearUpgradeCost(item),canUp=canUpgradeGear(item);
   return `<div class="gear-item rarity-${rarity} ${equipped?'gear-equipped':''}" data-gear="${item.id}" ${inventory?'draggable="true"':''}><div class="gear-top"><span class="gear-name">${item.name}</span><span class="gear-ilvl">${item.ilvl}</span></div><div class="gear-sub"><span class="gear-rarity">${quality.name}</span> · ${CLASS_INFO[item.classKey].badge} ${CLASS_INFO[item.classKey].name} · ${SLOT_ICONS[item.slot]} ${item.slot}</div><div class="gear-stats">${GEAR_STATS.filter(s=>values[s]>0).map(s=>`<span class="gear-stat">+${values[s]} ${s}</span>`).join('')}</div>${inventory?`<div class="gear-actions"><button class="equip" onclick="game.autoEquipGear('${item.id}')">${equipped?'Equipped':'Equip'}</button>${item.ilvl>=950&&item.ilvl<990?`<button class="equip" onclick="game.upgradeGear('${item.id}')" title="${canUp?'Upgrade this mythical piece':'Requires higher rating or max upgrade cap'}" ${canUp?'':'disabled'}>Upgrade ${next} · ${upCost} 💠</button>`:''}<button class="sell" onclick="game.sellGear('${item.id}')">Sell ${sell} 💠</button></div>`:''}</div>`;
  }
  renderCatalogue(){
 ...
 selectMenuChampionPreview(cls){
  ...
  this.menuPreviewPrestige=buildPrestigeVisual(cls,shownGear);if(this.menuPreviewPrestige)this.menuPreviewModel.add(this.menuPreviewPrestige);applyPrestigeWeaponIllusion(this.menuPreviewModel,cls,shownGear);
 ...
 updateMenuChampionPreview(dt){
  ...
  const canvas=$('#menuChampionCanvas'),w=Math.max(1,canvas.clientWidth),h=Math.max(1,canvas.clientHeight);
  if(canvas.width!==Math.floor(w*Math.min(devicePixelRatio*1.25,2.5))||canvas.height!==Math.floor(h*Math.min(devicePixelRatio*1.25,2.5))){this.menuPreviewRenderer.setSize(w,h,false);this.menuPreviewCamera.aspect=w/h;this.menuPreviewCamera.updateProjectionMatrix();}
...
renderMountJournal(){this.hideMountSkinTip?.();progression=unlockEligibleMounts(progression);saveProgression();const query=($('#mountSearch')?.value||'').toLowerCase(),filter=$('#mountFilter')?.value||'all';let mounts=MOUNT_CATALOGUE.filter(m=>`${m.name} ${m.desc} ${m.rarityLabel}`.toLowerCase().includes(query));if(filter==='owned')mounts=mounts.filter(m=>progression.mounts.includes(m.id));else if(filter==='locked')mounts=mounts.filter(m=>!progression.mounts.includes(m.id));else if(filter==='prestige')mounts=mounts.filter(m=>m.threshold);if(!mounts.length){$('#mountList').innerHTML='<div class="mount-panel-box">No mounts match this filter.</div>';return;}if(!mounts.some(m=>m.id===this.selectedMountId))this.selectedMountId=progression.activeMount&&mounts.some(m=>m.id===progression.activeMount)?progression.activeMount:mounts[0].id;const selected=mountDefinition(this.selectedMountId),owned=progression.mounts.includes(selected.id),highest=highestArenaRating(),active=mountDefinition(progression.activeMount);$('#mountSummary').innerHTML=`<div class="mount-pill">Collected<strong>${progression.mounts.length} / ${MOUNT_CATALOGUE.length}</strong></div><div class="mount-pill">Active Mount<strong>${active.name}</strong></div><div class="mount-pill">Highest Rating<strong>${highest}</strong></div><div class="mount-pill">Prestige<strong>${progression.mounts.filter(id=>mountDefinition(id).threshold).length}</strong></div>`;$('#mountList').innerHTML=mounts.map(m=>{const has=progression.mounts.includes(m.id);return `<button class="mount-row ${m.rarity} ${m.id===selected.id?'selected':''} ${has?'':'locked'}" data-mount="${m.id}"><div class="mount-icon">${m.icon}</div><div><div class="mount-name rarity-${m.rarity}">${m.name}</div><div class="mount-meta"><span class="rarity-chip rarity-${m.rarity}">${m.rarityLabel}</span> · ${m.id==='chronocrown_protodrake'?'Aether Cup colour schemes':(m.threshold?`${m.threshold} rating reward`:'Stable mount')}</div></div><span class="mount-state ${has?'owned':''}">${m.id===progression.activeMount?'Active':has?'Owned':'Locked'}</span></button>`;}).join('');$('#mountList').querySelectorAll('[data-mount]').forEach(row=>row.onclick=()=>{this.selectedMountId=row.dataset.mount;this.previewMountSkinId=progression.activeMountSkins?.[this.selectedMountId]||'default';this.renderMountJournal();this.setMountPreview(mountDefinition(this.selectedMountId));});$('#mountPreviewCopy').innerHTML=`<div class="rarity-chip rarity-${selected.rarity}">${selected.rarityLabel}</div><h3>${selected.name}</h3><p>${selected.skinLabel?`Colour: ${selected.skinLabel}`:selected.desc}</p><div class="mount-source">${owned?'Collected':'Locked'} · ${selected.source}</div>`;$('#mountFavourite').innerHTML=`<strong>${active.name}</strong>${selected.id===active.id?'Currently active in the arena.':'Only one favourite can be summoned at a time.'}`;$('#mountBadges').innerHTML=`<span class="rarity-${selected.rarity}">${selected.rarityLabel}</span><span>${owned?'Collected':'Locked'}</span><span>${selected.id==='chronocrown_protodrake'?'Aether Cup':(selected.threshold?'Rating Reward':'Baseline Stable')}</span>${selected.skinLabel?`<span>Skin: ${selected.skinLabel}</span>`:''}`;$('#mountDescription').textContent=selected.lore;$('#mountLore').innerHTML=`${selected.lore}<br><br>Mounts now use a larger silhouette and lift the rider visibly off the floor.`;$('#mountUnlock').innerHTML=selected.id==='chronocrown_protodrake'?`<div class="mount-panel-box"><strong>Unlock Requirement</strong>Win the Aether Cup on a class to unlock that class colour scheme.</div><div class="mount-panel-box"><strong>Reward Quality</strong><span class="rarity-${selected.rarity}">${selected.rarityLabel} mount</span> with class-based colour collection.</div>${selected.skins?this.mountSkinSelector(selected):''}`:selected.threshold?`<div class="mount-panel-box"><strong>Unlock Requirement</strong>Reach ${selected.threshold} rating.</div><div class="mount-panel-box"><strong>Reward Quality</strong><span class="rarity-${selected.rarity}">${selected.rarityLabel} mount</span> with enhanced aura and particles.</div>${selected.skins&&owned?this.mountSkinSelector(selected):''}`:`<div class="mount-panel-box"><strong>Unlock Requirement</strong>Available from the start.</div><div class="mount-panel-box"><strong>Selection Rule</strong>Own many; equip one favourite.</div>${selected.skins&&owned?this.mountSkinSelector(selected):''}`;$('#mountCollector').innerHTML=`<div class="mount-panel-box"><strong>Stable Progress</strong>${progression.mounts.length} of ${MOUNT_CATALOGUE.length} collected.</div><div class="mount-panel-box"><strong>Prestige Rewards</strong>2000: Infernal Warstrider<br>2200: Aether Deathcharger<br>2400: Aether Gladiator Wyrm<br>Tournament: Chronocrown Proto-Drake colour schemes<br>2700/3000: Wyrm colour schemes</div>`;const btn=$('#equipMountBtn');btn.disabled=!owned;btn.textContent=selected.id===progression.activeMount?'Active Mount Equipped':'Set Active Mount';btn.onclick=()=>{if(!owned)return;progression.activeMount=selected.id;saveProgression();if(this.player){this.player.createMountAppearance();this.renderActions();}this.renderMountJournal();this.refreshMenuProgress();};if(this.mountPreviewScene)this.setMountPreview(selected.skins&&this.previewMountSkinId?mountSkinPreviewDefinition(selected.id,this.previewMountSkinId):selected);}
 ...
 selectGuideClass(cls){
  ...
  $('#guidePassives').innerHTML=`<strong>Resource, passives & procs</strong>${CLASS_PASSIVES[cls]}`;
  $('#guideAbilities').innerHTML=AB[cls].map((a,i)=>{const cleanTip=String(a.tip||'').replace(/^Talent ability\.\s*/,'').replace(/^(?:Caster|Storm|Melee|Healer|Holy healer|Atonement healer|Martial|Affliction|Warrior) mechanic:\s*/i,'');return `<div class="guide-ability"><div class="guide-ability-head"><span class="guide-ability-name">${i+1}. ${abilityIcon(a)} ${a.name}</span><span class="guide-ability-meta">${a.cast?a.cast+'s CAST':'INSTANT'} · ${a.range?a.range+'m':'SELF'} · ${a.cd?a.cd+'s CD':'NO CD'}</span></div><p>${cleanTip} <span style="color:#9ccaff">Costs ${a.cost} ${info.resource==='energy'?'energy':'mana'}.</span></p></div>`;}).join('');
 ...
 updateGuidePreview(dt){if(!this.guideRenderer||$('#classGuide').classList.contains('hidden'))return;const canvas=$('#guideCanvas'),w=Math.max(1,canvas.clientWidth),h=Math.max(1,canvas.clientHeight);if(canvas.width!==Math.floor(w*Math.min(devicePixelRatio*1.5,3))||canvas.height!==Math.floor(h*Math.min(devicePixelRatio*1.5,3))){this.guideRenderer.setSize(w,h,false);this.guideCamera.aspect=w/h;this.guideCamera.updateProjectionMatrix();}if(this.guideModel&&!this.guideDragging){this.guideYaw+=dt*.48;this.guideModel.rotation.y=this.guideYaw;}else if(this.guideModel)this.guideModel.rotation.y=this.guideYaw;if(this.guideModel?.userData?.rig&&typeof AetherKit!=='undefined')AetherKit.pose(this.guideModel.userData.rig,{motion:0,phase:0,time:performance.now()*0.001},dt);this.guideRenderer.render(this.guideScene,this.guideCamera);}
 preview(){let mode=$('#modeSelect').value,chosen=$('#classSelect').value,cls=CLASS_INFO[chosen].name,arena=this.arena?.displayName||'Random Arena';const healer=['sage','pala','disc'].includes(chosen);if(healer){$('#composition').innerHTML=`<strong class="blue">Your team:</strong> ${cls} + Random DPS${mode==='3v3'?' + Random DPS':''}<br><strong class="red">Enemy team:</strong> Random Healer + Matching DPS${mode==='3v3'?' + Matching DPS':''}<br><small style="color:var(--muted)">Arena: <strong>${arena}</strong> · Healer matches mirror DPS classes for fair testing.</small>`;}else{$('#composition').innerHTML=`<strong class="blue">Your team:</strong> ${cls} + Random Healer${mode==='3v3'?' + Random DPS':''}<br><strong class="red">Enemy team:</strong> Random Healer + Random DPS${mode==='3v3'?' + Random DPS':''}<br><small style="color:var(--muted)">Arena: <strong>${arena}</strong> · Enemy compositions randomise each round.</small>`;}this.syncMenuCards();this.refreshMenuProgress();}
...
saveBinds(){localStorage.setItem('aetherBinds',JSON.stringify(binds));}
 ...
 classAbilityDisplayOrder(cls){
  ...
  const byName=new Map(defaults.map(item=>[item.a.name,item]));
  const saved=Array.isArray(abilityLayouts[cls])?abilityLayouts[cls]:[];
  const names=[];
  saved.forEach(name=>{if(byName.has(name)&&!names.includes(name))names.push(name);});
  defaults.forEach(item=>{if(!names.includes(item.a.name))names.push(item.a.name);});
  abilityLayouts[cls]=names.slice();
  return names.map((name,slot)=>({...byName.get(name),slot})).filter(Boolean);
 }
 swapAbilitySlots(fromSlot,toSlot){
  ...
  if(fromSlot>=ordered.length||toSlot>=ordered.length)return;
  const names=ordered.map(item=>item.a.name);
  const fromName=names[fromSlot],toName=names[toSlot];
 ...
 keydown(e){this.audio.ensure();if(!$('#armoury').classList.contains('hidden')){if(e.code==='Escape')this.closeArmoury();return;}if(!$('#encounterDetails').classList.contains('hidden')){if(e.code==='Escape')this.closeEncounterDetails();return;}if(!$('#classGuide').classList.contains('hidden')){if(e.code==='Escape')this.closeClassGuide();return;}if(!$('#achievements').classList.contains('hidden')){if(e.code==='Escape')this.closeAchievements();return;}if(e.code==='Backquote'){e.preventDefault();$('#debug').classList.toggle('hidden');this.updateDebug();return;}
  ...
  const actionLayout=this.classAbilityDisplayOrder(this.player.cls);for(let slot=0;slot<Math.min(16,actionLayout.length);slot++)if(combo===binds['a'+(slot+1)]){e.preventDefault();this.playerCast(actionLayout[slot].i);break;}
 ...
 cycleTarget(team){const arr=this.units.filter(u=>u.alive&&(team==='enemy'?u.team!==this.player.team:u.team===this.player.team));if(!arr.length)return;let i=arr.indexOf(this.target);this.target=arr[(i+1)%arr.length];if(this.player&&['sage','pala','disc'].includes(this.player.cls))this.healerManualAttackTarget=null;}
 pick(e){this.audio.ensure();if(this.phase!=='fight')return;const r=this.canvas.getBoundingClientRect();const px=e.clientX,py=e.clientY;this.mouse.x=((px-r.left)/r.width)*2-1;this.mouse.y=-((py-r.top)/r.height)*2+1;this.ray.setFromCamera(this.mouse,this.camera);
  ...
  if(chosen){
   const healerPlayer=this.player&&['sage','pala','disc'].includes(this.player.cls),autoAttackHealer=this.player&&['sage','pala'].includes(this.player.cls);
   if(autoAttackHealer&&chosen.team!==this.player.team){
    if(this.target===chosen){
     this.healerManualAttackTarget=this.healerManualAttackTarget===chosen?null:chosen;
     this.message(`${chosen.info.badge} Target: ${chosen.name} — ${this.healerManualAttackTarget===chosen?'healer auto-attack enabled':'healer auto-attack paused'}`);
    }else{
     this.target=chosen;this.healerManualAttackTarget=null;
     this.message(`${chosen.info.badge} Target: ${chosen.name} — ${chosen.info.name}. Click again to enable healer auto-attacks.`);
    }
   }else{
    this.target=chosen;if(healerPlayer)this.healerManualAttackTarget=null;
    this.message(`${chosen.info.badge} Target: ${chosen.name} — ${chosen.info.name}`);
   }
  }
 }
 isInCombat(u){return !!u&&u.combatUntil>this.time;}
 breakableControl(u){if(!u)return null;return u.effects.find(e=>['furyStun','cheapStun','stun','fear','poly','sleep','blind','windIncap','root'].includes(e.type)&&e.time>.18)||null;}
 ...
 tryBotEmergencyDefensive(u){
  ...
  if(u.cls==='disc'){const fade=this.abilityReady(u,['Fade','discFade']);if(fade>=0&&(hp<.48||major&&hp<.68))return this.tryAbility(u,fade,u,false)===true;}if(u.cls==='soul'){const wall=this.abilityReady(u,['Undying Resolve','undyingResolve']);if(wall>=0&&(hp<.62||major&&hp<.82||u.has('smokeBomb')||this.breakableControl(u)&&hp<.72))return this.tryAbility(u,wall,u,false)===true;const i=this.abilityReady(u,['Dark Pact','Void Mend']);if(i>=0&&(hp<.48||major&&hp<.66))return this.tryAbility(u,i,u,false)===true;}
  return false;
 }
 ...
 useTrinket(u=this.player,fromAI=false){if(!u||!u.alive||this.phase!=='fight')return false;if(u.trinketCd>0){if(!fromAI&&u===this.player)this.message(`Medallion ready in ${u.trinketCd.toFixed(1)}s`);return false;}const remove=e=>['furyStun','cheapStun','stun','fear','poly','sleep','blind','windIncap','root','slow','silence'].includes(e.type)||e.type.startsWith('lock_');const removed=u.effects.filter(remove);if(!removed.length&&fromAI)return false;u.trinketCd=60;this.float(u,'MEDALLION!', 'info');this.vfxGlyph(u,0xffd36b,.8);this.vfxNova(u,0xffe6a0,1.6,12);this.audio.play('proc');if(removed.length){u.effects=u.effects.filter(e=>!remove(e));u.cast=null;if(u===this.player)this.message("Gladiator's Medallion used — CC removed");else this.log(`${u.name} used Gladiator's Medallion.`);}else if(u===this.player){this.float(u,'MISSED!', 'error');this.message("Gladiator's Medallion used — no CC removed");}return true;}
 ...
 focusTargetMode(a){return ['heal','hot','shield','spiritBlossom','ironbark','bigHeal','cleanse','holyLight','sacrifice','bestowFaith','discShield','discMend','painSuppression'].includes(a?.type)?'ally':'enemy';}
 ...
 tryFocusBind(combo){if(!this.player)return false;const all=ensureFocusClass(this.player.cls);for(const [ability,rows] of Object.entries(all)){const macro=rows.find(m=>m.key&&m.key===combo);if(macro)return this.castFocusAbility(macro.enemySlot,+ability);}return false;}
  playerCast(i){this.castFor(this.player,i,this.target);}
  castFor(u,i,target){if(this.netGuest&&u===this.player){if(this.netSession)this.netSession.sendCast(i,target);return;}let a=AB[u.cls][i];if(!a)return;if(a.type==='discPenance'){if(!target||!target.alive)target=u;}else if(['buff','dash','defensive','shieldSelf','push','healerEscape','natureSwiftness','undyingResolve','monkDefensive','fistsChannel','ghanir','ultimateRadiance','discFade','discFear','archangel','darkArchangel','angelicBody','flameNova','paladinAoE','paladinGuard','paladinSteed','iceBlock','reflect','shout','warriorGuard'].includes(a.type))target=u;else if(a.type==='holyShock'){if(!target||!target.alive)target=u;}else if(['heal','hot','shield','spiritBlossom','ironbark','bigHeal','cleanse','holyLight','sacrifice','bestowFaith','discShield','discMend','painSuppression'].includes(a.type)){if(!target||target.team!==u.team)target=u;}else{if(!target||target.team===u.team||!target.alive)target=this.closestEnemy(u);}this.tryAbility(u,i,target,u===this.player);}
 ...
 applyTalentAbilityMods(c,a,index){
  ...
  if(c.cls==='sage'&&out.cast&&['Verdant Mend','Healing Wave','Lifebloom'].some(n=>out.name.includes(n)))out.cast=Math.max(.8,out.cast-r('quickmend')*.08);
  if(c.cls==='shadow'){
  ...
  if(c.cls==='wind'&&out.name==='Cloudstep Kick')out.cd=Math.max(6,out.cd-r('longdash')); if(c.has&&c.has('combustion')&&out.cast)out.cast=Math.max(.25,out.cast*.85);if(c.has&&c.has('windlordReady')&&out.name==='Cloudstep Kick'){out.value=Math.round((out.value||0)*1.15);out.tip=(out.tip||'')+' Strike of the Windlord empowered: +15% damage.';}
  return out;
 }

validate(caster,a,target,show,opts={}){if(!caster.alive)return false;if(caster.has('iceBlock')&&a.type!=='iceBlock')return this.fail(caster,'Immune inside Ice Block — cancel it first',show);if(!caster.has('bladestorm')&&a.type!=='iceBlock'&&(caster.has('furyStun')||caster.has('cheapStun')||caster.has('stun')||caster.has('fear')||caster.has('poly')||caster.has('sleep')||caster.has('blind')||caster.has('windIncap')))return this.fail(caster,'Crowd controlled',show);if((caster.has('silence')&&a.school!=='physical')||caster.has('lock_'+a.school))return this.fail(caster,'Spell locked',show);if(caster.cast&&!opts.ignoreCast)return this.fail(caster,'Already casting',show);if(caster.gcd>0&&!opts.ignoreGcd)return false;const cdIndex=Number.isInteger(opts.index)?opts.index:AB[caster.cls].indexOf(a);if(!opts.ignoreCd&&cdIndex>=0&&caster.cds[cdIndex]>0)return this.fail(caster,'Ability not ready',show);if(caster.resource<a.cost)return this.fail(caster,'Not enough resource',show);const self=['buff','dash','defensive','shieldSelf','push','healerEscape','natureSwiftness','undyingResolve','monkDefensive','fistsChannel','whirlingDragonPunch','ghanir','ultimateRadiance','discFade','discFear','archangel','darkArchangel','angelicBody','flameNova','paladinAoE','paladinGuard','paladinSteed','iceBlock','reflect','shout','warriorGuard','avatar','combustion','flameShield','evasion','cloak','totemMastery','stormkeeper','tigereyeBrew','karma','bladestorm','tigersLust'].includes(a.type);const friendly=['heal','hot','shield','spiritBlossom','ironbark','bigHeal','cleanse','holyLight','sacrifice','bestowFaith','discShield','discMend','painSuppression'].includes(a.type);if(!self&&(!target||!target.alive))return this.fail(caster,'No valid target',show);if(!self&&target&&isUntargetableStealth(target,caster))return this.fail(caster,'Target is stealthed',show);if(friendly&&target&&target.team!==caster.team)return this.fail(caster,'Target an ally',show);if(a.type==='sacrifice'&&target===caster)return this.fail(caster,'Choose an ally for Sacrifice',show);if(!self&&dist(caster,target)>a.range)return this.fail(caster,'Out of range',show); if(!self&&!this.arena.los(caster,target))return this.fail(caster,'Line of sight blocked',show);return true;}
 ...
 resolve(c,a,t,opts={}){this.animateAction(c,a);this.audio.playAbility(a,c);if(a.type==='volcanicEruption'&&t){this.vfxVolcanicEruption(t);this.audio.playSample('fire_impact',{gain:.72,rate:.78,cooldown:0,source:t});}if(t&&t!==c&&t.team!==c.team&&t.has('iceBlock')){this.float(t,'IMMUNE','info');return;}if(t&&t!==c&&t.team!==c.team&&t.has('cloakShadows')&&a.school!=='physical'&&!this.isMeleeStrike(a.name)&&!['Fists of Fury','Bladestorm'].includes(a.name)){this.float(t,'CLOAK IMMUNE','info');return;}if(c.cls==='storm'&&a.name!=='Arc Spark'&&a.name!=='Tempest Bolt'&&['chain','stun','root','push'].includes(a.type))c.effects=c.effects.filter(e=>e.type!=='arcSequence');let mult=(c.has('burst')?1.22:1)*(opts.proc?1.15:1)*(c.has('smokePower')?1.10:1);switch(a.type){
   ...
   case'unstableAffliction':{let ua=t.has('unstableAffliction');if(!ua){ua=t.effect('unstableAffliction',10,{value:50*mult,source:c,stacks:1});}else{ua.time=10;ua.tick=1;ua.source=c;ua.stacks=Math.min(3,(ua.stacks||1)+1);ua.value=([0,50,40,110/3][ua.stacks]||50)*mult;}this.vfxAfflictionApply(t,0xf06cff,'unstable');this.vfxNova(t,0xb85cff,1.45,8);this.float(t,`UNSTABLE AFFLICTION ×${ua.stacks||1}`,'info');break;}
   case'shiv':{const hit=this.damage(c,t,a.value*mult,a.name);if(hit){t.effect('slow',4,{pct:.65});t.effect('shivPoisonAmp',4,{source:c,pct:.30});this.float(t,'SHIV · 65% SLOW · POISON +30%','info');this.vfxKickArc(t,COLORS.shadow);}}break;    case'dot':{const empowered=!!c.has('venomEdge');if(a.name==='Garrote'){const hit=this.damage(c,t,Math.round(a.value*.70)*mult,a.name);if(hit){t.effect('bleed',8,{value:46*mult,source:c,label:'Garrote',interval:(t.has('vendetta')&&t.has('vendetta').source===c)?.5:1});this.vfxBurst(t,0x9b1c31,.72);this.vfxSpiral(t,0xd33b55,.82);this.float(t,'GARROTE · HEAVY BLEED','error');}}else if(empowered){c.effects=c.effects.filter(e=>e.type==='venomEdge');this.float(c,'VENOM FINISHER!','info');const hit=this.damage(c,t,(a.value+78)*mult,a.name);if(hit){t.effect('poison',6,{value:28*mult,source:c,interval:(t.has('vendetta')&&t.has('vendetta').source===c)?.5:1});this.vfxNova(t,0x7ce867,1.8,14);this.vfxSpiral(t,0xb84cff,1.0);this.audio.play('proc');}}else{const hit=this.damage(c,t,a.value*mult,a.name);if(hit){t.effect('poison',6,{value:14*mult,source:c,interval:(t.has('vendetta')&&t.has('vendetta').source===c)?.5:1});this.vfxBurst(t,0x8ce65b,.45);}}break;}    case'defensive':c.effect('defensive',4);if(c.cls==='shadow'){c.effect('smokePower',8);c.effect('cheapReady',8);c.effect('smokeBombReady',8);c.effect('stealth',8);this.float(c,'STEALTH · VEILED ASSAULT','info');this.vfxGlyph(c,0x7862a6,.78);}this.shieldBubble(c,0x7151a5,4);this.float(c,'VEIL','info');break;
   ...
   case'singleStun':{const hit=this.damage(c,t,a.value*mult,a.name);if(hit){this.applyStun(t,a.name==='Ribbreaker'?4:6);if(a.name==='Shadowfury'){c.effects=c.effects.filter(e=>e.type!=='pandemicSurge');c.effect('pandemicSurge',8,{pct:.20});this.float(c,'PANDEMIC BLOOM +20%','info');}c.effect('eviscerateReady',10);this.float(c,'EVISCERATE READY','info');this.vfxGlyph(c,0xc398ff,.72);this.vfxRing(t,0xb35bff,2.1);if(c.cls==='shadow'){c.effects=c.effects.filter(e=>e.type!=='stealth');}if(c.cls==='shadow'&&a.name==='Ribbreaker'&&progression?.talents?.shadow?.shadow_shadowstep){t.effect('bleed',6,{value:28.1*mult,source:c,label:'Internal Bleeding',interval:1});this.float(t,'INTERNAL BLEEDING','error');this.vfxBurst(t,0x9b1c31,.45);}if(c.has('smokeBombReady')){c.effects=c.effects.filter(e=>e.type!=='smokeBombReady');t.effect('smokeBomb',5);this.smokeBombShroud(t,5);this.float(t,'SMOKE BOMB — NO HEALS','error');this.log(`${c.name} shrouded ${t.name} in Smoke Bomb.`);}}break;}
   case'root':this.applyRoot(t,a.value||4);this.vfxRing(t,COLORS.storm,2.2);this.vfxGlyph(t,0x62d8ff,.85);this.vfxSpiral(t,0xa6f1ff,.7);break;
   case'shieldSelf':this.applyShield(c,c,a.value,6);if(c.cls==='storm'){c.effect('staticAegisGuard',6,{reduction:.20});this.float(c,'STATIC AEGIS · 20% WALL','info');}if(c.cls==='soul'){c.effect('interruptWard',6);this.vfxGlyph(c,COLORS.soul,.8);this.float(c,'INTERRUPT IMMUNE','info');}break;
   case'iceBlock':c.effects=c.effects.filter(e=>!['furyStun','cheapStun','stun','root','fear','poly','sleep','blind','windIncap','slow'].includes(e.type));c.effect('iceBlock',8,{value:c.maxHp*.025,source:c});c.cast=null;this.vfxIceBlock(c,8);this.vfxGlyph(c,0xdbf7ff,1.0);this.vfxRing(c,0xb9eaff,2.4);this.float(c,'ICE BLOCK — CC BROKEN','info');break;
   ...
   case'tigersLust':{
    ...
    case'avatar':{const rooted=!!c.has('root');c.effects=c.effects.filter(e=>e.type!=='root');c.effect('avatar',10,{damagePct:.16});this.vfxAvatarForm(c,10);this.vfxRing(c,COLORS.warrior,3.2);this.vfxNova(c,COLORS.warrior,2.2,18);this.vfxGlyph(c,COLORS.warrior,1.0);this.audio.play('warriorBlade',c);this.float(c,rooted?'AVATAR · ROOT BROKEN':'AVATAR · STONE FORM · +16% DAMAGE','info');break;}
   ...
   case'heal':this.heal(c,t,a.value,a.name);this.healBolt(c,t);break;
   ...
   case'bigHeal':this.heal(c,t,a.value,a.name);this.vfxRing(t,COLORS.heal,2.5);break;
   ...
   case'mortalSwing':{const emp=c.has('empoweredSwing'),wb=c.has('warbreakerReady');const wbMult=wb?1.30:1;if(wb){c.effects=c.effects.filter(e=>e!==wb);this.float(c,'WARBREAKER · NEXT SWING CONSUMED','info');this.vfxGlyph(c,COLORS.warrior,.72);}if(emp){const talentBonus=(talentRank(c.cls,'war_pummel_chain')||0)*.02;const v=a.value*(1.30+talentBonus)*wbMult*mult;this.damage(c,t,v,'Mortal Swing');this.vfxKickArc(t,COLORS.warrior);this.vfxNova(t,COLORS.warrior,1.15,8);c.effects=c.effects.filter(e=>e!==emp);this.float(c,'PUMMEL EMPOWERED · +30%','info');}else{this.damage(c,t,a.value*wbMult*mult,'Mortal Swing');this.vfxKickArc(t,COLORS.warrior);}break;}
   case'charge':{this.moveAdjacent(c,t,true);this.vfxTrail(c,COLORS.warrior);const hit=this.damage(c,t,a.value*mult,'Charge');if(hit){this.applyRoot(t,1.5);t.effect('slow',4,{pct:.45,source:c,label:'Charge Snare'});this.vfxRing(t,COLORS.warrior,1.7);this.float(t,'ROOTED → HAMSTRUNG','info');}this.float(c,'CHARGE!','info');break;}
 ...
 isMeleeStrike(label){return ['Night Slash','Eviscerate','Umbral Pounce','Viper Cut','Ribbreaker','Shadow Kick','Zephyr Palm','Cloudstep Kick','Dawncrest Kick','Cyclone Barrage','Fists of Fury','Disrupting Palm','Valley Sweep','Mortal Swing','Charge','Rend','Gushing Wound'].includes(label);}
 ...
 try{
  if(c&&c.cls==='soul')out*=.828;/* v201 additional Soulweaver damage nerf */
  ...
  if(c&&c.has&&c.has('darkArchangel'))out*=1.30;if(c&&c.has&&c.has('defensive')?.damagePenalty)out*=1-c.has('defensive').damagePenalty;if(c&&c.has&&c.has('avatar'))out*=1.16;
  if(c&&c.has&&c.has('combustion')&&Math.random()<.30){out*=1.5;this.float(c,'COMBUSTION CRIT','info');}
  ...
  if(c&&c.gearStats){
   ...
   out*=1+Math.min(.30,(c.gearStats.Power||0)*.00038+(c.gearStats.Versatility||0)*.00018+primary*.00010);
  }
 }catch(err){
 ...
 if(t.has('evasion')&&this.isMeleeStrike(label)&&Math.random()<.80){this.float(t,'DODGE','info');this.vfxRing(t,0xb27cff,1.25);this.vfxSpiral(t,0xdcc4ff,.45);return false;}
 const beforeHp=t.hp;
 ...
 return true;
}  heal(c,t,v,label){if(c&&c.gearStats){const primary=(c.gearStats.Intellect||0)+(c.gearStats.Agility||0)+(c.gearStats.Strength||0);v*=1+Math.min(.30,(c.gearStats.Restoration||0)*.00038+(c.gearStats.Versatility||0)*.00018+primary*.00010);}if(c&&c.cls)v*=classTalentHealingMult(c.cls,c);if(this.mode==='3v3'&&c&&['sage','pala','disc'].includes(c.cls))v*=1.20;if(c&&c.has&&c.has('ghanir')&&['Blooming Echo','Rejuvenate'].includes(label))v*=1.50;if(c&&c.cls==='pala')v*=1.16;if(c&&c.cls==='sage')v*=1.045;if(t&&t.has&&t.has('ironbark'))v*=1.20;if(c&&c.has&&c.has('totemMastery'))v*=1.03;if(c&&c.has('avengingWings'))v*=1.20;if(this.isInCombat(t))this.enterCombat(c,t);t.receiveHeal(c,v,label);}
 supportAttackTarget(u){if(u===this.player){if(['sage','pala'].includes(u.cls)){const t=this.healerManualAttackTarget;return t&&t.alive&&t===this.target&&t.team!==u.team&&!t.has('poly')&&!t.has('sleep')&&!t.has('blind')&&!t.has('windIncap')?t:null;}return this.target&&this.target.alive&&this.target.team!==u.team?this.target:null;}const allies=this.units.filter(a=>a.team===u.team&&a.alive);if(allies.some(a=>a.hp/a.maxHp<.86))return null;return this.units.filter(e=>e.team!==u.team&&e.alive&&!isUntargetableStealth(e,u)&&!e.has('poly')&&!e.has('sleep')&&!e.has('blind')&&!e.has('windIncap')).sort((a,b)=>dist(u,a)-dist(u,b))[0]||null;}
 updateSupportAttacks(){for(const u of this.units){if(!u.alive||!['pala','sage'].includes(u.cls)||u.basicAttackCd>0||u.cast||u.gcd>0||u.has('iceBlock')||u.has('stun')||u.has('fear')||u.has('poly')||u.has('sleep')||u.has('blind')||u.has('windIncap'))continue;const t=this.supportAttackTarget(u);if(!t||!this.arena.los(u,t))continue;if(u.cls==='pala'){if(dist(u,t)>3.35)continue;u.basicAttackCd=1;this.animateAction(u,{type:'damage',school:'holy'});this.vfxGlyph(t,COLORS.holy,.4);this.vfxBurst(t,COLORS.holy,.34);this.damage(u,t,66,'Righteous Strike');if(u===this.player)this.audio.play('holy');}else if(u.cls==='sage'){if(dist(u,t)>24)continue;u.basicAttackCd=1;this.animateAction(u,{type:'damage',school:'heal'});this.projectile(u,t,'heal',()=>this.damage(u,t,59,'Verdant Bolt'));if(u===this.player)this.audio.play('heal');}}}
 ...
 vfxAvatarForm(u,duration=10){
  ...
  u.modelGroup?.traverse?.(node=>{
   ...
   materials.filter(Boolean).forEach(material=>{
    ...
    originals.push({material,color:material.color.clone(),emissive:material.emissive?.clone?.()||null,roughness:material.roughness});
  ...
  let restored=false;
  const restore=()=>{if(restored)return;restored=true;originals.forEach(({material,color,emissive,roughness})=>{material.color.copy(color);if(emissive&&material.emissive)material.emissive.copy(emissive);if('roughness'in material)material.roughness=roughness;});};
 ...
 rollStormSurge(c){let ramp=c.has('stormChance');const chance=Math.min(1,(ramp?.chance||.10)+(c.has('totemMastery')?.05:0));if(Math.random()<chance){c.effects=c.effects.filter(e=>!['stormChance','stormCharge','arcSequence'].includes(e.type));c.effect('tempestBolts',10,{stacks:2});c.effect('overload',10);this.gainMana(c,10);this.float(c,`STORM SURGE! ×2 · +10 MANA`,'info');this.vfxGlyph(c,COLORS.storm,1.0);this.vfxOrbit(c,0xc1fbff,1.32);this.vfxNova(c,COLORS.storm,2.25,16);this.audio.play('proc');this.log(`${c.name} triggered Storm Surge, restored 10 mana and readied Volcanic Overload.`);}else{const next=Math.min(1,chance+.10);if(!ramp)ramp=c.effect('stormChance',16,{chance:next,stacks:`${Math.round(next*100)}%`});else{ramp.time=16;ramp.chance=next;ramp.stacks=`${Math.round(next*100)}%`;}if(c===this.player)this.float(c,`SURGE CHANCE ${Math.round(next*100)}%`,'info');}}
...
vfxIronbark(u,dur=6){if($('#reducedFX').checked)return;const group=new THREE.Group();const barkMat=new THREE.MeshBasicMaterial({color:0x6f4a28,transparent:true,opacity:.62,side:THREE.DoubleSide,depthWrite:false});const leafMat=new THREE.MeshBasicMaterial({color:0x7cff9d,transparent:true,opacity:.78,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending});for(let i=0;i<7;i++){const slab=new THREE.Mesh(new THREE.BoxGeometry(.16,.86,.055),barkMat);const a=i*Math.PI*2/7;slab.position.set(Math.cos(a)*.68,.62,Math.sin(a)*.68);slab.rotation.y=-a;group.add(slab);}const ring=new THREE.Mesh(new THREE.TorusGeometry(.82,.022,7,42),leafMat);ring.rotation.x=Math.PI/2;ring.position.y=.12;group.add(ring);for(let i=0;i<10;i++){const leaf=new THREE.Mesh(new THREE.ConeGeometry(.035,.18,5),leafMat);leaf.userData.phase=i*.6;group.add(leaf);}this.scene.add(group);let time=0;this.effects.push({obj:group,life:dur,follow:u,update:dt=>{time+=dt;group.position.set(u.x,0,u.z);ring.rotation.z+=dt*2.4;group.children.slice(8).forEach((leaf,i)=>{const a=time*(1.9+i*.03)+i*Math.PI/5;leaf.position.set(Math.cos(a)*(.86+.06*Math.sin(time+i)),.75+.32*Math.sin(time*2+i),Math.sin(a)*(.86+.06*Math.sin(time+i)));leaf.rotation.z=-a;leaf.material.opacity=.52+.26*Math.sin(time*4+i);});barkMat.opacity=.42+.14*Math.sin(time*5);}});}
  applyShield(c,t,v,dur){if(c?.has?.('totemMastery'))v*=1.05;v=Math.round(v*(1-this.dampening));t.shield=Math.max(t.shield,v);t.effect('shield',dur,{value:v});c.stats.absorb+=v;this.float(t,`+${v} WARD`,'heal');this.shieldBubble(t,COLORS.shield,dur);}
 applyCC(t,type,duration,label,category){if(t.has('iceBlock')){this.float(t,'IMMUNE','info');return false;}if(t.has('bladestorm')&&['stun','furyStun','root','slow'].includes(type)){this.float(t,'BLADESTORM IMMUNE','info');return false;}let now=this.time,dr=t.dr[category];if(now>dr.until){dr.level=0;}if(dr.level>=3){this.float(t,'IMMUNE','info');return false;}const scales=[1,.5,.25];duration*=scales[dr.level];dr.level++;dr.until=now+BALANCE.ccDRReset;if((type==='stun'||type==='furyStun'||type==='fear'||type==='poly'||type==='sleep'||type==='blind'||type==='windIncap')&&t.cast)t.cast=null;t.effect(type,duration);this.float(t,`${label} ${duration.toFixed(1)}s`,'info');return true;}
 ...
 applyBlind(t,duration){this.applyCC(t,'blind',duration,'BLIND','incap');}
 applyWindIncap(t,duration){this.applyCC(t,'windIncap',duration,'INCAPACITATE','incap');}
 applyRoot(t,duration){this.applyCC(t,'root',duration,'ROOT','root');}  applySnareDR(t,duration,pct,label='SNARE'){const ok=this.applyCC(t,'slow',duration,label,'root');if(ok){const e=t.has('slow');if(e)e.pct=pct;}return ok;}
 ...
 fxAllowed(weight=1){if(this.mode!=='3v3')return true;const bucket=Math.floor(performance.now()/250);if(this.fxBucket!==bucket){this.fxBucket=bucket;this.fxSpent=0;}if((this.fxSpent||0)+weight>48)return false;this.fxSpent=(this.fxSpent||0)+weight;return true;}
 ...
 toScreen(u){const v=new THREE.Vector3(u.visualX??u.x,2.8,u.visualZ??u.z).project(this.camera);return{x:(v.x*.5+.5)*innerWidth,y:(-.5*v.y+.5)*innerHeight};}
...
renderActions(){
 ...
 const primary=ordered.slice(0,7).map(makeButton).join('');
 const extras=ordered.slice(7).map(makeButton).join('');
 bar.innerHTML=`${extras?`<div class="actionbar-toolkit">${extras}</div>`:''}<div class="actionbar-main">${trinket}${primary}${mount}</div>`;
 $('#castWrap').classList.toggle('above-extra-row',!!extras);
 ...
 abilityCategory(a){if(['heal','hot','bigHeal','shield','cleanse','healerEscape','natureSwiftness','undyingResolve','holyLight','sacrifice','paladinAoE','paladinGuard','discShield','discMend','painSuppression','ultimateRadiance','discFade','archangel','darkArchangel','angelicBody','avengingWings'].includes(a.type))return ['Support','heal'];if(a.type==='holyShock')return ['Holy Shock · Heal / Damage','heal'];if(['poly','sleep','blind','fear','root','stun','singleStun','slow','interrupt','interruptProc','paladinStun','stormbolt'].includes(a.type))return ['Control','control'];if(['defensive','shieldSelf','buff','dash','push','paladinSteed'].includes(a.type))return ['Utility','mana'];if(['soulDot','agony','unstableAffliction','soulDrain'].includes(a.type))return ['Affliction','damage'];return ['Damage','damage'];}
 showAbilityTooltip(i,button){if(!this.player)return;const slot=Number.isInteger(Number(button?.dataset?.slot))?Number(button.dataset.slot):i;const a=AB[this.player.cls][i],tt=$('#abilityTooltip'),cat=this.abilityCategory(a),rect=button.getBoundingClientRect();const cast=a.cast?`${a.cast.toFixed(2)}s Cast`:'Instant';const range=a.range?`${a.range}m Range`:'Self';const resource=this.player.info.resource==='energy'?'Energy':'Mana';let active='';
  ...
  if(a.name==='Ember Lance'&&this.player.has('meteorLance'))active=`<div class="tt-active">☄️ Meteor Lance ×${this.player.has('meteorLance').stacks||2} Ready — Off-global rapid Ember Lance charges deal +8% damage</div>`;
 ...
 renderFrames(){this.frameInto($('#allyFrames'),this.units.filter(u=>u.team==='ally'));this.frameInto($('#enemyFrames'),this.units.filter(u=>u.team==='enemy'));}
 frameInto(el,units){el.innerHTML=units.map((u,i)=>`<div class="unit-frame glass ${u.team}" data-unit="${this.units.indexOf(u)}">${u.team==='enemy'?`<div class="enemy-utility"><div class="enemy-dr-mini"></div><div class="enemy-medallion" title="Gladiator's Medallion"><span>🏅</span><span class="ready-dot">RDY</span></div></div>`:''}<div class="cc-frame"></div><div class="uf-head"><span class="uf-name">${u.isPlayer&&equippedAchievementTitle()?`<span class="uf-title">${equippedAchievementTitle()}</span>`:''}${u.team==='enemy'?`<span class="arena-slot">E${i+1}</span>`:''}${u.name}</span><span class="uf-class">${classIcon(u.cls,u.info.badge)} ${u.info.short} <span class="role-tag">${u.info.role}</span></span></div><div class="bar"><div class="fill hp"></div><div class="shield-fill"></div><span class="bartext"></span></div><div class="bar"><div class="fill resource"></div></div><div class="effects"></div><div class="mini-cast hidden"><div class="mini-fill"></div><span class="mini-label"></span></div></div>`).join('');el.querySelectorAll('.unit-frame').forEach(f=>f.onclick=()=>this.target=this.units[+f.dataset.unit]);}
 effectChip(e){const meta=effectMeta(e.type), buff=meta.buff||['hot','shield','burst','defensive'].includes(e.type), cc=meta.cc||['poly','stun','silence'].includes(e.type); return `<span class="eff ${buff?'buff':''} ${cc?'cc':''}">${meta.icon} ${meta.label}${e.stacks?` ×${e.stacks}`:''} ${Math.ceil(e.time)}</span>`;}
 ...
 drMarkup(u){if(!u||!u.dr)return '';const rows=[{key:'stun',icon:'⛔',css:'stun',title:'Stun DR reset'},{key:'fear',icon:'😱',css:'incap',title:'Fear DR reset — separate from Hex'},{key:'incap',icon:(u.has('blind')?'👁️':u.has('windIncap')?'💫':u.has('poly')?'🐑':u.has('sleep')?'🌸':'🐑/👁️/💫'),css:'incap',title:'Incapacitate DR: Hex, Blind, Incapacitate and Slumber share this timer'},{key:'root',icon:'🕸️',css:'root',title:'Root DR reset'}];const visible=rows.map(row=>{const state=u.dr[row.key],remaining=Math.max(0,(state?.until||0)-this.time);if(!state||state.level<=0||remaining<=0)return '';const next=state.level>=3?'IMMUNE':state.level===2?'NEXT 25%':'NEXT 50%';return `<span class="dr-tracker ${row.css}" title="${row.title}">${row.icon} ${next} <span class="dr-time">${remaining.toFixed(1)}s</span></span>`;}).filter(Boolean).join('');return visible?`<span class="dr-stack">${visible}</span>`:'';}
 frameDRMarkup(u){if(!u||!u.dr)return '';const rows=[{key:'stun',icon:'⛔',css:'stun',title:'Stun diminishing returns'},{key:'fear',icon:'😱',css:'incap',title:'Fear DR — separate from Hex'},{key:'incap',icon:'🐑',css:'incap',title:'Incapacitate DR: Hex / Blind / Incapacitate / Slumber'},{key:'root',icon:'🕸️',css:'root',title:'Root diminishing returns'}];return rows.map(row=>{const state=u.dr[row.key],remaining=Math.max(0,(state?.until||0)-this.time);if(!state||state.level<=0||remaining<=0)return '';const tier=state.level>=3?'IMM':state.level===2?'¼':'½';return `<span class="mini-dr ${row.css}" title="${row.title}: ${remaining.toFixed(1)} sec until reset">${row.icon}<b>${tier}</b><em>${remaining.toFixed(0)}s</em></span>`;}).filter(Boolean).join('');}
 renderMeter(box,stat,mode){if(!box||!this.units.length)return;const units=this.units.slice().sort((a,b)=>b.stats[stat]-a.stats[stat]);const max=Math.max(1,...units.map(u=>u.stats[stat]));const total=Math.max(1,units.reduce((sum,u)=>sum+u.stats[stat],0));box.innerHTML=units.map((u,i)=>{const value=Math.round(u.stats[stat]);const width=value/max*100;const pct=value/total*100;const unitIndex=this.units.indexOf(u);return `<div class="details-row clickable" data-report-mode="${mode}" data-report-unit="${unitIndex}" title="Open Encounter Details"><div class="details-fill ${mode==='healing'?'heal ':''}${u.team}" style="width:${width}%"></div><span class="details-rank">${i+1}.</span><span class="details-name"><span class="details-class">${classIcon(u.cls,u.info.badge)}</span>${u.name}</span><span class="details-value ${mode==='healing'?'heal-value':''}">${value.toLocaleString()} · ${pct.toFixed(0)}%</span></div>`;}).join('');box.onpointerdown=e=>{const row=e.target.closest('.details-row');if(!row||!box.contains(row))return;e.preventDefault();e.stopPropagation();this.openEncounterDetails(row.dataset.reportMode,+row.dataset.reportUnit);};}
 ...
 previewAbilityTarget(a){
  ...
  const self=['buff','dash','defensive','shieldSelf','push','healerEscape','monkDefensive','ghanir','flameNova','paladinAoE','paladinGuard','paladinSteed','avengingWings','iceBlock','reflect','shout','warriorGuard','combustion','flameShield','evasion','cloak','totemMastery','stormkeeper','tigereyeBrew','karma','bladestorm','tigersLust'].includes(a.type);
  const friendly=['heal','hot','shield','spiritBlossom','ironbark','bigHeal','cleanse','holyLight','sacrifice','bestowFaith','discShield','discMend','painSuppression'].includes(a.type);
  if(self)return null;
  if(a.type==='holyShock'){if(!target||!target.alive)target=this.player;}
  else if(friendly){if(!target||target.team!==this.player.team||!target.alive)target=this.player;}
 ...
 applyStealthVisibility(){
  ...
  updateUI(){this.applyStealthVisibility?.();this.updateTotemMasteryVisuals?.();this.updateCombustionVisuals?.();if(this.phase==='menu')return;this.updateUtilityUI();this.updateDetailsMeter();$('#timer').textContent=fmt(this.time);let ally=this.units.filter(u=>u.team==='ally'&&u.alive).length,enemy=this.units.filter(u=>u.team==='enemy'&&u.alive).length;$('#allyAlive').textContent=ally;$('#enemyAlive').textContent=enemy;
  ...
  document.querySelectorAll('.unit-frame').forEach(f=>{const u=this.units[+f.dataset.unit];if(!u)return;f.classList.toggle('selected',this.target===u);f.classList.toggle('dead',!u.alive);f.querySelector('.hp').style.width=(u.hp/u.maxHp*100)+'%';f.querySelector('.shield-fill').style.left=(u.hp/u.maxHp*100)+'%';f.querySelector('.shield-fill').style.width=(u.shield/u.maxHp*100)+'%';f.querySelector('.bartext').textContent=`${Math.ceil(u.hp)} / ${u.maxHp}${u.shield?`  +${Math.ceil(u.shield)}`:''}`;const rf=f.querySelector('.resource');rf.className='fill resource '+(u.info.resource==='energy'?'energy':'mana');rf.style.width=(u.resource/u.maxResource*100)+'%';f.querySelector('.effects').innerHTML=u.effects.slice(0,5).map(e=>this.effectChip(e)).join('');const med=f.querySelector('.enemy-medallion');if(med){const cd=Math.max(0,u.trinketCd||0);med.classList.toggle('used',cd>0);med.innerHTML=cd>0?`<span>🏅</span><span class="trinket-time">${Math.ceil(cd)}</span>`:`<span>🏅</span><span class="ready-dot">RDY</span>`;med.title=cd>0?`Gladiator's Medallion ready in ${Math.ceil(cd)}s`:`Gladiator's Medallion ready`;}const drRail=f.querySelector('.enemy-dr-mini');if(drRail)drRail.innerHTML=this.frameDRMarkup(u);const ccf=f.querySelector('.cc-frame');const ccHtml=this.ccMarkup(u);ccf.innerHTML=ccHtml;ccf.classList.toggle('show',!!ccHtml);let cs=f.querySelector('.mini-cast'),visibleCast=this.visibleFrameCast(u);cs.classList.toggle('hidden',!visibleCast);cs.classList.toggle('interrupt-shielded',!!u.has('interruptWard'));if(visibleCast){cs.querySelector('.mini-fill').style.width=((1-visibleCast.left/visibleCast.total)*100)+'%';cs.querySelector('.mini-label').innerHTML=this.castNameMarkup(visibleCast);}});
  const platePositions=new Map(this.units.map(u=>[u,this.toScreen(u)]));const stackingPlates=progression.settings?.stackingNameplates!==false;this.units.forEach(u=>u.plate.classList.remove('auto-stacked'));if(stackingPlates&&this.player){const hostile=this.units.filter(u=>u.team!==this.player.team&&u.alive).map(u=>({u,p:platePositions.get(u)})).sort((a,b)=>a.p.y-b.p.y);const groups=[];hostile.forEach(item=>{const group=groups.find(g=>g.some(other=>Math.abs(other.p.x-item.p.x)<118&&Math.abs(other.p.y-item.p.y)<64));if(group)group.push(item);else groups.push([item]);});groups.filter(g=>g.length>1).forEach(g=>{g.sort((a,b)=>a.p.y-b.p.y);const bottomY=Math.max(...g.map(x=>x.p.y));g.forEach((item,i)=>{platePositions.set(item.u,{...item.p,y:bottomY-i*58});item.u.plate.classList.add('auto-stacked');});});}this.units.forEach(u=>{let p=platePositions.get(u)||this.toScreen(u);u.plate.style.left=p.x+'px';u.plate.style.top=p.y+'px';u.plate.style.opacity=(u.alive?((u.has('stealth')&&this.player&&u.team!==this.player.team)?0:(u.has('stealth')?.65:1)):0);u.plate.classList.toggle('selected',u===this.target);u.plate.querySelector('.plate-name').innerHTML=`${u.isPlayer?`<span class="plate-identity">${equippedAchievementTitle()?`<span class="plate-title">${equippedAchievementTitle()}</span>`:''}<span>${u.name}</span></span>`:`<span>${u.name}</span>`}<span class="plate-class">${classIcon(u.cls,u.info.badge)} ${u.info.short}</span>`;u.plate.querySelector('.hp').style.width=(u.hp/u.maxHp*100)+'%';u.plate.querySelector('.shield-fill').style.left=(u.hp/u.maxHp*100)+'%';u.plate.querySelector('.shield-fill').style.width=(u.shield/u.maxHp*100)+'%';const pc=u.plate.querySelector('.plate-cast'),visibleCast=this.visibleFrameCast(u);pc.classList.toggle('hidden',!visibleCast);pc.classList.toggle('interrupt-shielded',!!u.has('interruptWard'));if(visibleCast){pc.querySelector('.plate-cast-head').innerHTML=this.castNameMarkup(visibleCast);pc.querySelector('.plate-cast-track span').style.width=((1-visibleCast.left/visibleCast.total)*100)+'%';}const pcc=u.plate.querySelector('.plate-cc');const ccHtml=this.ccMarkup(u);pcc.innerHTML=ccHtml;pcc.classList.toggle('show',u!==this.player&&!!ccHtml);});
  const tf=$('#targetFrame');if(this.target&&this.target.alive){const targetLos=this.target===this.player||this.arena.los(this.player,this.target);const targetCC=this.ccMarkup(this.target);tf.classList.remove('hidden');tf.classList.toggle('enemy-target',this.target.team!==this.player.team);tf.classList.toggle('blocked',!targetLos);this.target.plate.classList.toggle('los-blocked',!targetLos);const targetDR=this.drMarkup(this.target);tf.innerHTML=`<div class="uf-head"><b>${this.target.isPlayer&&equippedAchievementTitle()?`<span class=\"uf-title\">${equippedAchievementTitle()}</span>`:''}${this.target.name}</b><span class="uf-class">${classIcon(this.target.cls,this.target.info.badge)} ${this.target.info.name} <span class="role-tag">${this.target.info.role}</span></span></div><div class="target-health-row"><div class="bar"><div class="fill hp" style="width:${this.target.hp/this.target.maxHp*100}%"></div><div class="shield-fill" style="left:${this.target.hp/this.target.maxHp*100}%;width:${this.target.shield/this.target.maxHp*100}%"></div><span class="bartext">${Math.ceil(this.target.hp)} / ${this.target.maxHp}</span></div>${targetDR}</div><div class="effects">${this.target.effects.map(e=>this.effectChip(e)).join('')}</div>${this.targetCastMarkup(this.target)}${targetCC?`<div class="cc-frame show" style="position:relative;top:auto;right:auto;margin-top:7px;display:inline-flex">${targetCC}</div>`:''}<span class="los-pill ${targetLos?'':'blocked'}">${targetLos?'● IN SIGHT':'✕ LINE OF SIGHT'}</span>`;}else{tf.classList.add('hidden');tf.classList.remove('enemy-target');this.units.forEach(u=>u.plate.classList.remove('los-blocked'));}
 ...
 unitMoveByIntent(u,dt){if(!u||!u.alive||u.has('furyStun')||u.has('cheapStun')||u.has('stun')||u.has('poly')||u.has('sleep')||u.has('blind')||u.has('windIncap')||u.has('fear')||u.has('iceBlock')||u.has('root'))return;if(u.cast&&u.cast.channel&&!u.cast.moveCast)return;const v=u.intent;if(!v)return;if(u.cast&&!u.cast.moveCast){u.cast=null;if(u===this.player)this.message('Cast cancelled by movement');}let slow=u.has('slow')?.pct||0;/* Tiger's Lust is already included in Character.moveSpeed; do not multiply it twice. */const castMove=u.cast?.moveSpeedMult||1;u.x+=v.x*u.moveSpeed*(1-slow)*castMove*dt;u.z+=v.z*u.moveSpeed*(1-slow)*castMove*dt;if(!u.has('bladestorm'))u.mesh.rotation.y=Math.atan2(v.x,v.z);if(u===this.player)this.cameraRig.facingYaw=Math.atan2(-v.x,-v.z);this.arena.constrain(u);}
 ...
 returnMenu(){this.phase='menu';this.paused=false;this.forceTournamentStart=false;this.clear();$('#pauseMenu').classList.add('hidden');$('#settings').classList.add('hidden');$('#hud').classList.add('hidden');$('#overlay').classList.add('hidden');$('#menu').classList.remove('hidden');this.selectMenuChampionPreview($('#classSelect')?.value||'flame');this.refreshMenuProgress();}
 update(dt){if(this.netGuest){if(this.netSession)this.netSession.guestFrame(dt);}else{if(this.phase==='countdown'&&!this.paused){this.count-=dt;if(this.count<=0){this.phase='fight';this.message('FIGHT!');}else this.message(`Begins in ${Math.ceil(this.count)}`);}
  if(this.phase==='fight'&&!this.paused){this.time+=dt;if(this.time>=BALANCE.dampStart)this.dampening=clamp((Math.floor((this.time-BALANCE.dampStart)/BALANCE.dampInterval)+1)*BALANCE.dampStep,0,BALANCE.dampCap);this.playerMove(dt);this.units.forEach(u=>{u.update(dt);if(u.ai)u.ai.update(dt);});this.updateSupportAttacks();const aliveAlly=this.units.some(u=>u.team==='ally'&&u.alive),aliveEnemy=this.units.some(u=>u.team==='enemy'&&u.alive);if(!aliveEnemy)this.finish(true);if(!aliveAlly)this.finish(false);}if(this.netSession)this.netSession.hostFrame(dt);}
  this.effects.forEach(e=>{e.life-=dt;if(e.update)e.update(dt);if(e.follow&&!e.follow.alive)e.dead=true;});this.effects=this.effects.filter(e=>{if(e.life<=0||e.obj.dead||e.dead){this.scene.remove(e.obj);this.queueDispose(e.obj);return false;}return true;});const fxCap=this.mode==='3v3'?110:230;if(this.effects.length>fxCap){const extra=this.effects.splice(0,this.effects.length-fxCap);extra.forEach(e=>{if(e.obj){this.scene.remove(e.obj);this.queueDispose(e.obj);}});}this.flushDisposals(this.mode==='3v3'?14:5);
  this.updateUI();}
 updateCamera(dt){if(!this.player||this.phase==='menu')return;const r=this.cameraRig,focus=new THREE.Vector3(this.player.visualX??this.player.x,1.35,this.player.visualZ??this.player.z);const planar=Math.cos(r.pitch)*r.distance;const desired=new THREE.Vector3(focus.x+Math.sin(r.yaw)*planar,focus.y+Math.sin(r.pitch)*r.distance+1.25,focus.z+Math.cos(r.yaw)*planar);this.camera.position.lerp(desired,1-Math.pow(.0005,dt));this.camera.lookAt(focus.x,focus.y+.2,focus.z);}
 animate(){requestAnimationFrame(()=>this.animate());let dt=Math.min(.04,this.clock.getDelta());this.update(dt);this.updateCamera(dt);this.updateMenuChampionPreview(dt);this.updateGuidePreview(dt);this.updateArmouryPreview(dt);if(this.shake>0){this.shake-=dt;this.camera.position.x+=(Math.random()-.5)*.065;this.camera.position.y+=(Math.random()-.5)*.04;}this.renderer.render(this.scene,this.camera);}
}
...
window.AETHER_ONLINE_BRIDGE={CLASS_INFO:CLASS_INFO,AB:AB,AIController:AIController,getProgression:()=>progression};
