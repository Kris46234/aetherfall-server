  ...
  function resolveAbility(source, ability, target) {
    ...
    switch (ability.type) {
      ...
      case 'mortalSwing': {
        ...
        const multiplier = (empowered ? 1.15 : 1) * (warbreaker ? 1.15 : 1);
  ...
  function tickEffects(unit) {
    for (const [type, effect] of unit.effects) {
      ...
      if (effect.remaining === 0 && type === 'bestowFaith') {
        const source = state.units.get(effect.sourceId);
        if (source?.alive && unit.alive) heal(source, unit, effect.value || 240, 'Bestow Faith');
      }
      if (effect.remaining === 0 && type === 'livingBomb') {
        const source = state.units.get(effect.sourceId);
        if (source?.alive && unit.alive) damage(source, unit, effect.explosion || 190, 'Living Bomb Explosion', { school: 'fire' });
      }
