...
function Session(game){
 ...
 this.unitId=null;this.host=false;this.players=[];this.ready=false;this.started=false;this.closed=false;
...
Session.prototype.connect=function(url,room,creating){
 this.url=socketUrl(url);this.room=String(room||'').toUpperCase();this.closed=false;this.reconnectAttempts=0;
...
Session.prototype.open=function(creating){
 ...
 ws.onopen=function(){
  ...
  self.send({type:'join',protocol:PROTOCOL,roomCode:self.room,clientId:self.clientId,classId:selectedClass(),talents:talentsFor(selectedClass()),sessionToken:self.sessionToken||null});
...
Session.prototype.start=function(){if(!this.host){this.status('Only Player 1 can start the match.',true);return;}this.send({type:'start'});this.status('Starting the shared server match…');};
Session.prototype.changeClass=function(){if(this.started)return;this.send({type:'class',classId:selectedClass(),talents:talentsFor(selectedClass())});};
Session.prototype.refreshLobby=function(){
 var roster=$id('anRoster'),start=$id('anStart');
 if(start){start.disabled=!(this.host&&this.ready&&!this.started);start.textContent=this.started?'Match running':'Start Co-op';}
 if(!roster)return;
 var p1=this.players.filter(function(p){return p.slot==='player1';})[0],p2=this.players.filter(function(p){return p.slot==='player2';})[0];
 function line(label,p){return '<div>'+label+': <b>'+(p?className(p.classId)+(p.connected?' · connected':' · reconnecting'):'waiting…')+'</b></div>';}
 roster.innerHTML=line('Player 1',p1)+line('Player 2',p2)+'<div style="opacity:.75;margin-top:4px">Co-op only · both humans are equal clients against two server-controlled bots.</div>';
};
Session.prototype.onMessage=function(message){
 if(message.type==='joined'){
  ...
  this.unitId=message.unitId;this.host=!!message.host;this.sessionToken=message.sessionToken||'';this.players=message.players||[];
 ...
 if(message.type==='lobby'){
  this.players=message.players||[];this.ready=!!message.ready;this.refreshLobby();
  if(!this.started)this.status(this.ready?(this.host?'Player 2 connected — Start Co-op is ready.':'Both players connected — waiting for Player 1.'):'Waiting for the second player…');
  return;
 }
 if(message.type==='startAck'){if(!message.ok)this.status('Could not start: '+reasonText(message.reason),true);return;}
 if(message.type==='matchStart'){this.spawn(message.world,message.phase,message.countdownRemaining);return;}
 ...
 if(message.type==='matchEnd'){this.endMatch();return;}
...
Session.prototype.spawn=function(world,phase,countdownRemaining){
 ...
 var g=this.g,self=this;this.lastWorld=world;
 g.queueType='skirmish';g.mode='2v2';try{g.setArenaTheme(world.arena&&world.arena.theme||'runestone');}catch(e){}
 ...
 world.units.forEach(function(su){
  var team=su.team==='allies'?'ally':'enemy';
  var name=su.id==='player1'?'Player 1':su.id==='player2'?'Player 2':su.id==='bot1'?'Vael':'Mira';
  var unit=g.spawn(name,su.classId,team,su.x,su.z,su.id===self.unitId);unit.netId=su.id;unit.ai=null;
 });
...
Session.prototype.applySnapshot=function(world,phase,countdownRemaining){
 ...
 world.units.forEach(function(su){var unit=self.byId(su.id);if(!unit)return;
  ...
  if(sampleDt>.001&&sampleDt<.5){vx=(su.x-previous.x)/sampleDt;vz=(su.z-previous.z)/sampleDt;var speed=Math.hypot(vx,vz),limit=8;if(speed>limit){vx=vx/speed*limit;vz=vz/speed*limit;}}
...
Session.prototype.presentComplete=function(unit,event){
 ...
 try{g.animateAction(unit,ability);g.audio.playAbility(ability,unit);var type=ability.type||'',school=ability.school||'',color=school==='fire'?0xff7038:school==='holy'?0xffe28a:school==='shadow'?0xa875ff:school==='wind'?0x72e5a5:school==='nature'||school==='heal'?0x72ffc0:school==='storm'?0x72dfff:0x9edcff;
  ...
  if(target&&['root','paladinStun','windIncap','blind'].indexOf(type)>=0){g.vfxGlyph(target,color,.82);g.vfxRing(target,color,2.1);g.vfxSpiral(target,color,.72);return;}
...
Session.prototype.resolveTarget=function(ability,target){
 ...
 var selfTypes=['buff','dash','defensive','shieldSelf','push','healerEscape','natureSwiftness','undyingResolve','monkDefensive','fistsChannel','whirlingDragonPunch','ghanir','ultimateRadiance','discFade','discFear','archangel','darkArchangel','angelicBody','flameNova','paladinGuard','paladinSteed','avengingWings','iceBlock','reflect','shout','warriorGuard','avatar','combustion','flameShield','evasion','cloak','totemMastery','stormkeeper','tigereyeBrew','karma','bladestorm','tigersLust'];
 var friendly=['heal','hot','shield','spiritBlossom','ironbark','bigHeal','cleanse','holyLight','sacrifice','bestowFaith','discShield','discMend','painSuppression'];
...
Session.prototype.sendCast=function(index,target){
 var p=this.g.player,ability=p&&(AB[p.cls]||[])[index];if(!p||!ability||!this.started)return false;if(ability.type==='whirlingDragonPunch'){var fists=(AB[p.cls]||[]).findIndex(function(spell){return spell.type==='fistsChannel';});if(fists<0||(p.cds[fists]||0)<=0){this.g.message('Whirling Dragon Punch requires Fists of Fury to be on cooldown.');return false;}}if(ability.type==='volcanicEruption'&&!p.has('volcanicEruptionReady')){this.g.message('Skybreaker Pulse must ready Volcanic Eruption first.');return false;}target=this.resolveTarget(ability,target);var sequence=++this.actionSequence,now=performance.now();
 var packet={type:'action',sequence:sequence,abilityId:abilityId(p.cls,ability),targetId:target&&target.netId||null};
...
Session.prototype.sendInput=function(dt){
 this.inputTimer-=dt;var p=this.g.player,intent=p&&p.intent,key=intent?intent.x.toFixed(3)+','+intent.z.toFixed(3):'0';
 if(key!==this.lastInput||this.inputTimer<=0){this.inputTimer=.05;this.lastInput=key;this.send({type:'input',sequence:++this.inputSequence,x:intent?intent.x:0,z:intent?intent.z:0});}
};
Session.prototype.frame=function(dt){
 ...
 if(g.phase==='fight'&&g.player&&g.player.alive){g.player.intent=g.localMoveIntent();if(g.player.intent)g.unitMoveByIntent(g.player,dt);this.sendInput(dt);}
 g.units.forEach(function(unit){var target=unit.netTarget;if(target&&unit.alive){var own=unit===g.player,age=Math.min(.12,Math.max(0,(now-target.receivedAt)/1000)),tx=target.x+(own?0:target.vx*age),tz=target.z+(own?0:target.vz*age),gap=Math.hypot(tx-unit.x,tz-unit.z);if(own){var moving=!!unit.intent,ackLag=Math.max(0,self.inputSequence-Number(target.inputSequence||0));if(gap>3){unit.x=target.x;unit.z=target.z;}else if(!moving&&gap>.02){var settle=Math.min(1,dt*20);unit.x+=(target.x-unit.x)*settle;unit.z+=(target.z-unit.z)*settle;}else if(moving&&ackLag<=2&&gap>.22){var correction=Math.min(1,dt*5.5);unit.x+=(target.x-unit.x)*correction;unit.z+=(target.z-unit.z)*correction;}}else{var blend=Math.min(1,dt*18);unit.x+=(tx-unit.x)*blend;unit.z+=(tz-unit.z)*blend;}}unit.update(dt);});
...
Session.prototype.endMatch=function(){if(!this.started)return;var allyAlive=this.g.units.some(function(u){return u.team==='ally'&&u.alive;});this.started=false;this.g.netGuest=false;this.g.netSession=null;try{this.g.finish(allyAlive);}catch(e){};};
...
function buildUI(game){
 ...
 var panel=document.createElement('div');panel.id='anPanel';panel.className='hidden';panel.innerHTML='<div id="anCard"><button id="anClose">✕</button><h2>PLAY ONLINE — AUTHORITATIVE CO-OP</h2><div style="opacity:.8">Protocol 15 · Two human allies versus two talent-enabled tactical bots. Both players connect to one shared match; neither browser is the host authority.</div><div class="an-row"><input id="anServerUrl" placeholder="Authoritative server URL"></div><div class="an-row"><button id="anCreate">Create Co-op Lobby</button></div><div class="an-row" id="anHostRow" style="display:none"><input id="anRoomOut" readonly><button id="anCopy">Copy Invite</button><button id="anStart" disabled>Start Co-op</button></div><div class="an-row"><input id="anJoinIn" placeholder="Paste the co-op invite link"><button id="anJoin">Join / Retry</button><button id="anReset">Disconnect</button></div><div id="anRoster"></div><div id="anStatus"></div></div>';document.body.appendChild(panel);
 ...
 var saved='';try{saved=localStorage.getItem('aetherAuthorityUrl')||localStorage.getItem('aetherRelayUrl')||'';}catch(e){}$id('anServerUrl').value=saved||'https://aetherfall-authoritative-coop.onrender.com';
 ...
 $id('anCreate').onclick=function(){unlockAudio(game);var url=$id('anServerUrl').value.trim();if(!url){session().status('Paste the server URL first.',true);return;}try{localStorage.setItem('aetherAuthorityUrl',url);}catch(e){}var code=roomCode(),s=session();s.connect(url,code,true);var invite=location.origin+location.pathname+'#coop='+code+'&server='+encodeURIComponent(socketUrl(url));$id('anRoomOut').value=invite;$id('anHostRow').style.display='flex';};
 ...
 $id('anJoin').onclick=function(){unlockAudio(game);var value=$id('anJoinIn').value.trim(),match=value.match(/#coop=([A-Za-z0-9_-]+)&server=([^&]+)/);if(!match){session().status('That is not an authoritative co-op invite.',true);return;}var url=decodeURIComponent(match[2]);$id('anServerUrl').value=url;try{localStorage.setItem('aetherAuthorityUrl',url);}catch(e){}session().connect(url,match[1],false);};
 ...
 var hash=location.hash.match(/#coop=([A-Za-z0-9_-]+)&server=([^&]+)/);if(hash){showLobby();$id('anJoinIn').value=location.hash;setTimeout(function(){$id('anJoin').click();},350);}
