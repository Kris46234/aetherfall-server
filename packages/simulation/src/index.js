...
export function createSimulation({
  ...
  function applyAction(unitId, action) {
    ...
    const alterTimeReturn = ability.type === 'alterTime' && !!combat.getEffect(unit, 'alterTime');
    if (!ability.ignoreCooldown && !alterTimeReturn && (unit.cooldowns.get(ability.id) || 0) > 0) return rejectAction(unit, action, 'cooldown');
    if (ability.type === 'whirlingDragonPunch' && (unit.cooldowns.get('wind.fists_of_fury') || 0) <= 0) {
      return rejectAction(unit, action, 'requires_fists_cooldown');
    }
    if (ability.type === 'volcanicEruption' && !combat.getEffect(unit, 'volcanicEruptionReady')) {
      return rejectAction(unit, action, 'requires_skybreaker_pulse');
    }
    ...
    if (!target && action.targetId) {
      ...
      if (target.team !== unit.team && combat.getEffect(target, 'stealth')) return rejectAction(unit, action, 'target_stealthed');
    ...
    if (!alterTimeReturn && ability.cooldown > 0 && !ability.commitCooldownOnComplete) unit.cooldowns.set(ability.id, ability.cooldown);
    ...
    if (['fistsChannel', 'bladestorm', 'soulDrain', 'discPenance'].includes(ability.type)) {
      ...
      if (soulDrain) {
        const affliction = combat.getEffect(target, 'unstableAffliction');
        if (affliction) {
          affliction.remaining = 10;
          emit({
            type: 'effectRefreshed', unitId: target.id,
            effect: 'unstableAffliction', duration: 10,
            sourceId: unit.id, abilityId: ability.id
          });
        }
      }
      ...
      if (unit.cast) {
        ...
        if (unit.cast.remaining === 0) {
          ...
          if (!completed.channel) {
            ...
            const hostileStealth = target?.team !== unit.team && combat.getEffect(target, 'stealth');
