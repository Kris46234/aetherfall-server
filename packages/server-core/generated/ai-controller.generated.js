...
/* Extracted from apps/client/src/game-runtime.js (sha256 93329b3ff0f25619829f580106f3ecfb87dda0284a0d5548e733853bafef0e74) */
export const aiSourceDigest = "93329b3ff0f25619829f580106f3ecfb87dda0284a0d5548e733853bafef0e74";
...
class AIController {
 ...
 duelSurvivalResponse(enemies,d){
  const u=this.u,target=enemies.filter(e=>e.alive).sort((a,b)=>dist(u,a)-dist(u,b))[0];if(!target)return false;
  const hp=u.hp/u.maxHp,targetHp=target.hp/target.maxHp,enemyBurst=!!this.game.unitUnderMajorOffensive?.(u),ownBurst=!!this.game.majorOffensiveActiveOnUnit?.(u);
  const commit=ownBurst||targetHp<.43||targetHp<hp-.16;
  let used=false;
  if(u.cls==='flame'){
   ...
   if(!used&&(hp<.45||enemyBurst&&hp<.62))used=this.selfSaveAbility(['Evasion','Cloak of Shadows','Smoke Veil']);
  }else if(u.cls==='wind'){
   ...
   const victory=AB[u.cls].findIndex(a=>a.name==='Victory Rush');
   if(victory>=0&&u.cds[victory]<=0&&u.resource>=(AB[u.cls][victory].cost||0)&&hp<.70&&dist(u,target)<=4.8&&this.game.arena.los(u,target))used=this.game.tryAbility(u,victory,target)===true;
   if(!used&&(hp<.41||enemyBurst&&hp<.60))used=this.selfSaveAbility(['Shield Wall','warriorGuard']);
  }else if(u.cls==='storm'){
   if(hp<.47||enemyBurst&&hp<.62)used=this.selfSaveAbility(['Static Aegis','Grounding Aegis']);
  }else if(u.cls==='soul'){
   if(hp<.31)used=this.selfSaveAbility(['Undying Resolve']);
   if(!used&&(hp<.56||enemyBurst&&hp<.68))used=this.selfSaveAbility(['Dark Pact']);
  }
  if(used){this.focus=target;this.duelKiteUntil=0;return true;}
  if(commit){this.duelKiteUntil=0;return false;}
  const ranged=['flame','storm','soul'].includes(u.cls),needsBriefReset=ranged&&enemyBurst&&hp<.66&&dist(u,target)<13;
  if(needsBriefReset&&this.duelKiteUntil<=this.game.time)this.duelKiteUntil=this.game.time+.9;
  if(needsBriefReset&&this.game.time<this.duelKiteUntil){this.moveToward(target,true);this.kitePressure(target);return true;}
  this.duelKiteUntil=0;
  return false;
 }
